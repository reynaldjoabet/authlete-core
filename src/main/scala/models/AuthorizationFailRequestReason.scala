...
/** The reason of the failure of the authorization request. For more details,
  ...
  */
enum AuthorizationFailRequestReason(val value: String)
    ...
    withValueOpt(value).getOrElse(
      throw java.lang
        .IllegalArgumentException(
          s"AuthorizationFailRequestReason enum case not found: $value"
        )
    )

  given decoderAuthorizationFailRequestReason
      : Decoder[AuthorizationFailRequestReason] = Decoder.decodeString
    .map(withValue)

  given encoderAuthorizationFailRequestReason
      : Encoder[AuthorizationFailRequestReason] = Encoder.encodeString
