...
/** @param accessToken
  ...
  *   Unix epoch (1970-01-01). If the `accessTokenExpiresAt` request parameter
  *   is not included in a request or its value is 0 (or negative), the
  *   expiration date of the access token is not changed.
  * @param scopes
  ...
  *   with the access token is not allowed to request are ignored on the server
  *   side. If the `scopes` request parameter is not included in a request or
  *   its value is `null`, the scopes of the access token are not changed. Note
  *   that `properties` parameter is accepted only when `Content-Type` of the
  *   request is `application/json`, so don't use
  ...
  *   update the expiration date of the access token when the scopes linked to
  ...
  *   known (perhaps from lookup) but the value of the token itself is not. The
  *   value of the `accessToken` parameter takes precedence.
  * @param accessTokenValueUpdated
  *   A boolean request parameter which indicates whether to update the value of
  *   the access token in the data store. If this parameter is set to `true`
  *   then a new access token value is generated by the server and returned in
  *   the response.
  * @param accessTokenPersistent
  *   The flag which indicates whether the access token expires or not. By
  *   default, all access tokens expire after a period of time determined by
  *   their service. If this request parameter is `true` then the access token
  *   will not automatically expire and must be revoked or deleted manually at
  *   the service. If this request parameter is `true`, the
  *   `accessTokenExpiresAt` request parameter is ignored. If this request
...
object TokenUpdateRequest {
  ...
  given jsonCodec: JsonValueCodec[TokenUpdateRequest] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderTokenUpdateRequest: Encoder[TokenUpdateRequest] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
          ...
          t.accessTokenValueUpdated.map(v =>
            "accessTokenValueUpdated" -> v.asJson
          ),
  ...
  given decoderTokenUpdateRequest: Decoder[TokenUpdateRequest] =
    Decoder.instance { c =>
      for {
        ...
        accessTokenExpiresAtUpdatedOnScopeUpdate <-
          c.downField("accessTokenExpiresAtUpdatedOnScopeUpdate")
            .as[Option[Boolean]]
        accessTokenHash <- c.downField("accessTokenHash").as[Option[String]]
        accessTokenValueUpdated <- c
          .downField("accessTokenValueUpdated")
          .as[Option[Boolean]]
