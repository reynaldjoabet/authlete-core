...
/** @param resultCode
  ...
  * @param subject
  *   The subject who is associated with the access token. The value of this
  *   property is `null` if the access token was issued using the flow of
  ...
  *   token which had been issued along with the access token. `false` if the
  *   refresh token for the access token has expired or the access token has no
  ...
  *   `resource` request parameters or by the `resource` property in the request
  *   object. See \"Resource Indicators for OAuth 2.0\" for details.
  * @param accessTokenResources
  *   The target resources this proeprty holds may be the same as or different
  *   from the ones `resource` property holds. In some flows, the initial
  *   request and the subsequent token request are sent to different endpoints.
  *   Example flows are the Authorization Code Flow, the Refresh Token Flow, the
  *   CIBA Ping Mode, the CIBA Poll Mode and the Device Flow. In these flows,
  *   not only the initial request but also the subsequent token request can
  *   include the `resource` request parameters. The purpose of the `resource`
  *   request parameters in the token request is to narrow the range of the
  *   target resources from the original set of target resources requested by
  *   the preceding initial request. If narrowing down is performed, the target
  *   resources holded by the `resource` proeprty and the ones holded by this
  *   property are different. This property holds the narrowed set of target
  *   resources. See \"Resource Indicators for OAuth 2.0\" for details.
  ...
  * @param scopeDetails
  *   The scopes property of this class is a list of scope names. The property
  *   does not hold information about scope attributes. This scopeDetails
  *   property was newly created to convey information about scope attributes.
  * @param grantId
...
object IntrospectionResponse {
  ...
  given jsonCodec: JsonValueCodec[IntrospectionResponse] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderIntrospectionResponse: Encoder[IntrospectionResponse] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
  ...
  given decoderIntrospectionResponse: Decoder[IntrospectionResponse] =
    Decoder.instance { c =>
      for {
        ...
        action <- mapEmptyStringToNull(c.downField("action"))
          .as[Option[IntrospectionResponseAction]]
