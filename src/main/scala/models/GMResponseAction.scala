...
object GMResponseAction {
  ...
  def withValueOpt(value: String): Option[GMResponseAction] =
    GMResponseAction.values
      .find(_.value == value)

  def withValue(value: String): GMResponseAction =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"GMResponseAction enum case not found: $value"
      )
    )

  given decoderGMResponseAction: Decoder[GMResponseAction] =
    Decoder.decodeString.map(withValue)

  given encoderGMResponseAction: Encoder[GMResponseAction] =
