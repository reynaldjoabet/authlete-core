...
/** Values for the `client_registration_types` RP metadata and the
  ...
  */
enum ClientRegistrationType(val value: String)
...
object ClientRegistrationType {
  ...
  def withValueOpt(value: String): Option[ClientRegistrationType] =
    ...
    withValueOpt(value).getOrElse(
      throw java.lang
        .IllegalArgumentException(
          s"ClientRegistrationType enum case not found: $value"
        )
    )

  given decoderClientRegistrationType: Decoder[ClientRegistrationType] =
    Decoder.decodeString
      .map(withValue)

  given encoderClientRegistrationType: Encoder[ClientRegistrationType] =
