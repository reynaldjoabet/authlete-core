...
/** The reason of the failure of the token request.
  */
enum TokenFailRequestReason(val value: String)
...
object TokenFailRequestReason {
  ...
  def withValueOpt(value: String): Option[TokenFailRequestReason] =
    TokenFailRequestReason.values
      .find(_.value == value)
    ...
    withValueOpt(value).getOrElse(
      throw java.lang
        .IllegalArgumentException(
          s"TokenFailRequestReason enum case not found: $value"
        )
    )

  given decoderTokenFailRequestReason: Decoder[TokenFailRequestReason] =
    Decoder.decodeString
      .map(withValue)

  given encoderTokenFailRequestReason: Encoder[TokenFailRequestReason] =
