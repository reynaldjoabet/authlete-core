...
/** @param number
  ...
  * @param serviceNumber
  *   The sequential number of the service of the client application. The value
  ...
  *   `client_name` in [OpenID Connect Dynamic Client Registration 1.0, 2.
  *   Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param clientNames
  *   Client names with language tags. If the client application has different
  *   names for different languages, this property can be used to register the
  *   names.
  * @param description
  *   The description about the client application.
  * @param descriptions
  *   Descriptions about the client application with language tags. If the
  *   client application has different descriptions for different languages,
  ...
  *   letters). The value of this property is assigned by Authlete. Note that
  *   Authlete issues a client secret even to a \"public\" client application,
  *   but the client application should not use the client secret unless it
  *   changes its client type to \"confidential\". That is, a public client
  *   application should behave as if it had not been issued a client secret. To
  *   be specific, a token request from a public client of Authlete should not
  *   come along with a client secret although [RFC 6749, 3.2.1. Client
  *   Authentication](https://datatracker.ietf.org/doc/html/rfc6749#section-3.2.1)
  *   says as follows. > Confidential clients or other clients issued client
  *   credentials MUST authenticate with the authorization server as described
  ...
  *   only when this client's `clientIdAliasEnabled` property is set to `true`
  *   AND the service's `clientIdAliasEnabled` property is also set to `true`.
  * @param clientIdAliasEnabled
  *   The flag to indicate whether the client ID alias is enabled or not. Note
  *   that a service also has `clientIdAliasEnabled` property. If the service's
  *   `clientIdAliasEnabled` property is set to `false`, the client ID alias of
  *   this client is not recognized even if this client's `clientIdAliasEnabled`
  *   property is set to `true`.
  * @param clientType
  * @param applicationType
  * @param logoUri
  *   The URL pointing to the logo image of the client application. This
  *   property corresponds to `logo_uri` in [OpenID Connect Dynamic Client
  *   Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param logoUris
  *   Logo image URLs with language tags. If the client application has
  *   different logo images for different languages, this property can be used
  *   to register URLs of the images.
  * @param contacts
  *   An array of email addresses of people responsible for the client
  *   application. This property corresponds to contacts in [OpenID Connect
  *   Dynamic Client Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param tlsClientCertificateBoundAccessTokens
  *   The flag to indicate whether this client use TLS client certificate bound
  *   access tokens.
  ...
  *   to be dynamically registered. This property corresponds to the
  *   `software_id metadata` defined in [2. Client
  *   Metadata](https://datatracker.ietf.org/doc/html/rfc7591#section-2) of [RFC
  *   7591](https://datatracker.ietf.org/doc/html/rfc7591).
  * @param softwareVersion
  *   The version identifier string for the client software identified by the
  *   software ID. This property corresponds to the software_version metadata
  *   defined in [2. Client
  *   Metadata](https://datatracker.ietf.org/doc/html/rfc7591#section-2) of [RFC
  *   7591](https://datatracker.ietf.org/doc/html/rfc7591).
  ...
  * @param createdAt
  *   The time at which this client was created. The value is represented as
  *   milliseconds since the UNIX epoch (1970-01-01).
  * @param modifiedAt
  *   The time at which this client was last modified. The value is represented
  *   as milliseconds since the UNIX epoch (1970-01-01).
  * @param grantTypes
  *   A string array of grant types which the client application declares that
  *   it will restrict itself to using. This property corresponds to
  *   `grant_types` in [OpenID Connect Dynamic Client Registration 1.0, 2.
  *   Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param responseTypes
  ...
  *   `response_types` in [OpenID Connect Dynamic Client Registration 1.0, 2.
  *   Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param redirectUris
  *   Redirect URIs that the client application uses to receive a response from
  *   the authorization endpoint. Requirements for a redirect URI are as
  *   follows. **Requirements by RFC 6749** (From [RFC 6749, 3.1.2. Redirection
  *   Endpoint](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1.2)) -
  *   Must be an absolute URI. - Must not have a fragment component.
  *   **Requirements by OpenID Connect** (From \"[OpenID Connect Dynamic Client
  *   Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata),
  *   application_type\") - The scheme of the redirect URI used for Implicit
  *   Grant by a client application whose application is `web` must be `https`.
  *   This is checked at runtime by Authlete. - The hostname of the redirect URI
  *   used for Implicit Grant by a client application whose application type is
  *   `web` must not be `localhost`. This is checked at runtime by Authlete. -
  *   The scheme of the redirect URI used by a client application whose
  *   application type is `native` must be either (1) a custom scheme or (2)
  *   `http`, which is allowed only when the hostname part is `localhost`. This
  *   is checked at runtime by Authlete. **Requirements by Authlete** - Must
  *   consist of printable ASCII letters only. - Must not exceed 200 letters.
  *   Note that Authlete allows the application type to be `null`. In other
  *   words, a client application does not have to choose `web` or `native` as
  *   its application type. If the application type is `null`, the requirements
  *   by OpenID Connect are not checked at runtime. An authorization request
  *   from a client application which has not registered any redirect URI fails
  *   unless at least all the following conditions are satisfied. - The client
  *   type of the client application is `confidential`. - The value of
  *   `response_type` request parameter is `code`. - The authorization request
  *   has the `redirect_uri` request parameter. - The value of `scope` request
  *   parameter does not contain `openid`. RFC 6749 allows partial match of
  *   redirect URI under some conditions (see [RFC 6749, 3.1.2.2. Registration
  ...
  *   The string representation of the expected subject distinguished name of
  *   the certificate this client will use in mutual TLS authentication. See
  *   `tls_client_auth_subject_dn` in \"Mutual TLS Profiles for OAuth Clients,
  *   2.3. Dynamic Client Registration\" for details.
  * @param tlsClientAuthSanDns
  *   The string representation of the expected DNS subject alternative name of
  *   the certificate this client will use in mutual TLS authentication. See
  *   `tls_client_auth_san_dns` in \"Mutual TLS Profiles for OAuth Clients, 2.3.
  *   Dynamic Client Registration\" for details.
  * @param tlsClientAuthSanUri
  *   The string representation of the expected URI subject alternative name of
  *   the certificate this client will use in mutual TLS authentication. See
  *   `tls_client_auth_san_uri` in \"Mutual TLS Profiles for OAuth Clients, 2.3.
  *   Dynamic Client Registration\" for details.
  ...
  *   See `tls_client_auth_san_ip` in \"Mutual TLS Profiles for OAuth Clients,
  *   2.3. Dynamic Client Registration\" for details.
  ...
  *   authentication. See `tls_client_auth_san_email` in \"Mutual TLS Profiles
  *   for OAuth Clients, 2.3. Dynamic Client Registration\" for details.
  * @param parRequired
  ...
  *   `require_pushed_authorization_requests` client metadata defined in \"OAuth
  *   2.0 Pushed Authorization Requests\".
  * @param requestObjectRequired
  *   The flag to indicate whether authorization requests from this client are
  *   always required to utilize a request object by using either `request` or
  *   `request_uri` request parameter. If this flag is set to `true` and the
  *   service's `traditionalRequestObjectProcessingApplied` is set to `false`,
  ...
  *   requires that URLs used as values for `request_uri` request parameter be
  *   pre-registered. This property is used for the pre-registration. See
  *   [OpenID Connect Core 1.0, 6.2. Passing a Request Object by
  *   Reference](https://openid.net/specs/openid-connect-core-1_0.html#RequestUriParameter)
  *   for details.
  * @param defaultMaxAge
  *   The default maximum authentication age in seconds. This value is used when
  *   an authorization request from the client application does not have
  *   `max_age` request parameter. This property corresponds to
  *   `default_max_age` in [OpenID Connect Dynamic Client Registration 1.0, 2.
  *   Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param defaultAcrs
  *   The default ACRs (Authentication Context Class References). This value is
  *   used when an authorization request from the client application has neither
  *   `acr_values` request parameter nor `acr` claim in claims request
  *   parameter.
  ...
  *   embedded in the ID token. This property corresponds to `require_auth_time`
  ...
  *   `sector_identifier_uri` client metadata which is defined in [OpenID
  ...
  *   `sector_identifier_uri` or the registered redirect URI. If no
  *   `sector_identifier_uri` is registered and multiple redirect URIs are also
  *   registered, the value of this property is `null`.
  * @param jwksUri
  *   The URL pointing to the JWK Set of the client application. The content
  *   pointed to by the URL is JSON which complies with the format described in
  *   [JSON Web Key (JWK), 5. JWK Set
  *   Format](https://datatracker.ietf.org/doc/html/rfc7517#section-5). The JWK
  *   Set must not include private keys of the client application. If the client
  *   application requests encryption for ID tokens (from the
  *   authorization/token/userinfo endpoints) and/or signs request objects, it
  *   must make available its JWK Set containing public keys for the encryption
  *   and/or the signature at the URL of `jwksUri`. The service (Authlete)
  *   fetches the JWK Set from the URL as necessary. [OpenID Connect Dynamic
  *   Client Registration
  *   1.0](https://openid.net/specs/openid-connect-registration-1_0.html) says
  *   that `jwks` must not be used when the client can use `jwks_uri`, but
  *   Authlete allows both properties to be registered at the same time.
  *   However, Authlete does not use the content of `jwks` when `jwksUri` is
  *   registered. This property corresponds to `jwks_uri` in [OpenID Connect
  *   Dynamic Client Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param jwks
  *   The content of the JWK Set of the client application. The format is
  *   described in [JSON Web Key (JWK), 5. JWK Set
  *   Format](https://datatracker.ietf.org/doc/html/rfc7517#section-5). The JWK
  *   Set must not include private keys of the client application. [OpenID
  ...
  *   application. This property corresponds to `initiate_login_uri` in [OpenID
  *   Connect Dynamic Client Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param tosUri
  *   The URL pointing to the \"Terms Of Service\" page. This property
  *   corresponds to `tos_uri` in [OpenID Connect Dynamic Client Registration
  *   1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param tosUris
  ...
  *   end-user's profile data is used. This property corresponds to `policy_uri`
  *   in [OpenID Connect Dynamic Client Registration 1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param policyUris
  *   URLs of policy pages with language tags. If the client application has
  *   different policy pages for different languages, this property can be used
  *   to register the URLs.
  * @param clientUri
  *   The URL pointing to the home page of the client application. This property
  *   corresponds to `client_uri` in [OpenID Connect Dynamic Client Registration
  *   1.0, 2. Client
  *   Metadata](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata).
  * @param clientUris
  *   Home page URLs with language tags. If the client application has different
  *   home pages for different languages, this property can be used to register
  *   the URLs.
  * @param bcDeliveryMode
  *   The backchannel token delivery mode. This property corresponds to the
  *   `backchannel_token_delivery_mode` metadata. The backchannel token delivery
  *   mode is defined in the specification of \"CIBA (Client Initiated
  *   Backchannel Authentication)\".
  * @param bcNotificationEndpoint
  *   The backchannel client notification endpoint. This property corresponds to
  *   the `backchannel_client_notification_endpoint` metadata. The backchannel
  *   token delivery mode is defined in the specification of \"CIBA (Client
  *   Initiated Backchannel Authentication)\".
  * @param bcRequestSignAlg
  * @param bcUserCodeRequired
  *   The boolean flag to indicate whether a user code is required when this
  ...
  *   The authorization details types that this client may use as values of the
  *   `type` field in `authorization_details`. This property corresponds to the
  *   `authorization_details_types` metadata. See [OAuth 2.0 Rich Authorization
  *   Requests (RAR)](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar/)
  *   for details. Note that the property name was renamed from
  *   authorizationDataTypes to authorizationDetailsTypes to align with the
  *   change made by the 5th draft of the RAR specification.
  * @param customMetadata
  *   The custom client metadata in JSON format. Standard specifications define
  *   client metadata as necessary. The following are such examples. * [OpenID
  *   Connect Dynamic Client Registration
  ...
  *   Registration/Update Request is set to the clientName property. On the
  *   other hand, unrecognized client metadata are discarded. By listing up
  *   custom client metadata in advance by using the
  *   `supportedCustomClientMetadata` property of Service, Authlete can
  *   recognize them and stores their values into the database. The stored
  ...
  *   is a different flag, `requestObjectRequired`, for the purpose. Even if
  *   this flag is `false`, encryption of request object is required if the
  *   `frontChannelRequestObjectEncryptionRequired` flag of the service is
  *   `true`.
  * @param requestObjectEncryptionAlgMatchRequired
  *   The flag indicating whether the JWE alg of encrypted request object must
  *   match the `request_object_encryption_alg` client metadata. The
  *   `request_object_encryption_alg` client metadata itself is defined in
  ...
  *   match the `request_object_encryption_enc` client metadata. The
  *   `request_object_encryption_enc` client metadata itself is defined in
  *   [OpenID Connect Dynamic Client Registration
  *   1.0](https://openid.net/specs/openid-connect-registration-1_0.html) as
  ...
  *   property that represents the client metadata is `requestEncryptionEnc`.
  *   See the description of `requestEncryptionEnc` for details. Even if this
  *   flag is `false`, the match is required if the
  ...
  *   or userinfo responses (or any place that can have the `verified_claims`
  *   claim). Possible values are listed in the <a href=
  *   \"https://www.iana.org/assignments/named-information/named-information.xhtml#hash-alg\"
  *   >Hash Algorithm Registry</a> of IANA (Internet Assigned Numbers
  *   Authority), but the server does not necessarily support all the values
  *   there. When this property is omitted, `sha-256` is used as the default
  *   algorithm. This property corresponds to the `digest_algorithm` client
  *   metadata which was defined by the third implementer's draft of [OpenID
  ...
  *   combination of subject and client. Note that, however, attempts by Client
  *   Credentials Flow do not invalidate existing access tokens because access
  *   tokens issued by Client Credentials Flow are not associated with any
  *   end-user's subject. Even if `Disabled` is selected here, single access
  *   token per subject is effective if `singleAccessTokenPerSubject` of the
  *   `Service` this client belongs to is Enabled.
  * @param pkceRequired
  ...
  *   The flag indicating whether this client was registered by the \"explicit\"
  *   client registration of OIDC Federation.
  ...
  *   this client. This property corresponds to the organization_name client
  ...
  *   the JWT format. This property corresponds to the `signed_jwks_uri` client
  *   metadata defined in OpenID Connect Federation 1.0.
  * @param entityId
  *   the entity ID of this client.
  * @param trustAnchorId
  *   The entity ID of the trust anchor of the trust chain that was used when
  *   this client was registered or updated by the mechanism defined in OpenID
  *   Connect Federation 1.0
  * @param trustChain
  *   The trust chain that was used when this client was registered or updated
  *   by the mechanism defined in OpenID Connect Federation 1.0
  * @param trustChainExpiresAt
  *   the expiration time of the trust chain that was used when this client was
  *   registered or updated by the mechanism defined in OpenID Connect
  *   Federation 1.0. The value is represented as milliseconds elapsed since the
  *   Unix epoch (1970-01-01).
  * @param trustChainUpdatedAt
  *   the time at which the trust chain was updated by the mechanism defined in
...
object Client {
  ...
  given jsonCodec: JsonValueCodec[Client] =
  ...
  given encoderClient: Encoder[Client] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
        ...
        t.registrationAccessTokenHash.map(v =>
          "registrationAccessTokenHash" -> v.asJson
        ),
        ...
        t.authorizationEncryptionAlg.map(v =>
          "authorizationEncryptionAlg" -> v.asJson
        ),
        t.authorizationEncryptionEnc.map(v =>
          "authorizationEncryptionEnc" -> v.asJson
        ),
        t.tokenAuthMethod.map(v => "tokenAuthMethod" -> v.asJson),
        t.tokenAuthSignAlg.map(v => "tokenAuthSignAlg" -> v.asJson),
        t.selfSignedCertificateKeyId.map(v =>
          "selfSignedCertificateKeyId" -> v.asJson
        ),
        ...
        t.derivedSectorIdentifier.map(v =>
          "derivedSectorIdentifier" -> v.asJson
        ),
        ...
        t.authorizationDetailsTypes.map(v =>
          "authorizationDetailsTypes" -> v.asJson
        ),
        ...
        t.automaticallyRegistered.map(v =>
          "automaticallyRegistered" -> v.asJson
        ),
        ...
        t.clientRegistrationTypes.map(v =>
          "clientRegistrationTypes" -> v.asJson
        ),
  ...
  given decoderClient: Decoder[Client] = Decoder.instance { c =>
    for {
      ...
      clientIdAliasEnabled <- c
        ...
        .as[Option[Boolean]]
      clientType <- mapEmptyStringToNull(c.downField("clientType"))
        .as[Option[ClientType]]
      applicationType <-
        mapEmptyStringToNull(c.downField("applicationType"))
          .as[Option[ApplicationType]]
      ...
      dynamicallyRegistered <- c
        .downField("dynamicallyRegistered")
        .as[Option[Boolean]]
      softwareId <- c.downField("softwareId").as[Option[String]]
      softwareVersion <- c.downField("softwareVersion").as[Option[String]]
      registrationAccessTokenHash <- c
        .downField("registrationAccessTokenHash")
        .as[Option[String]]
      ...
      responseTypes <- c
        .downField("responseTypes")
        .as[Option[Seq[ResponseType]]]
      redirectUris <- c.downField("redirectUris").as[Option[Seq[String]]]
      authorizationSignAlg <-
        mapEmptyStringToNull(c.downField("authorizationSignAlg"))
          .as[Option[JwsAlg]]
      authorizationEncryptionAlg <- mapEmptyStringToNull(
        c.downField("authorizationEncryptionAlg")
      )
        .as[Option[JweAlg]]
      authorizationEncryptionEnc <- mapEmptyStringToNull(
        c.downField("authorizationEncryptionEnc")
      )
        .as[Option[JweEnc]]
      tokenAuthMethod <-
        mapEmptyStringToNull(c.downField("tokenAuthMethod"))
          .as[Option[ClientAuthMethod]]
      tokenAuthSignAlg <- mapEmptyStringToNull(c.downField("tokenAuthSignAlg"))
        .as[Option[JwsAlg]]
      selfSignedCertificateKeyId <- c
        .downField("selfSignedCertificateKeyId")
        .as[Option[String]]
      tlsClientAuthSubjectDn <- c
        .downField("tlsClientAuthSubjectDn")
        .as[Option[String]]
      tlsClientAuthSanDns <- c
        .downField("tlsClientAuthSanDns")
        .as[Option[String]]
      tlsClientAuthSanUri <- c
        .downField("tlsClientAuthSanUri")
        .as[Option[String]]
      tlsClientAuthSanIp <- c.downField("tlsClientAuthSanIp").as[Option[String]]
      tlsClientAuthSanEmail <- c
        .downField("tlsClientAuthSanEmail")
        .as[Option[String]]
      parRequired <- c.downField("parRequired").as[Option[Boolean]]
      requestObjectRequired <- c
        .downField("requestObjectRequired")
        .as[Option[Boolean]]
      requestSignAlg <- mapEmptyStringToNull(c.downField("requestSignAlg"))
        .as[Option[JwsAlg]]
      requestEncryptionAlg <-
        mapEmptyStringToNull(c.downField("requestEncryptionAlg"))
          .as[Option[JweAlg]]
      requestEncryptionEnc <-
        mapEmptyStringToNull(c.downField("requestEncryptionEnc"))
          .as[Option[JweEnc]]
      ...
      idTokenSignAlg <- mapEmptyStringToNull(c.downField("idTokenSignAlg"))
        .as[Option[JwsAlg]]
      idTokenEncryptionAlg <-
        mapEmptyStringToNull(c.downField("idTokenEncryptionAlg"))
          .as[Option[JweAlg]]
      idTokenEncryptionEnc <-
        mapEmptyStringToNull(c.downField("idTokenEncryptionEnc"))
          .as[Option[JweEnc]]
      authTimeRequired <- c.downField("authTimeRequired").as[Option[Boolean]]
      subjectType <- mapEmptyStringToNull(c.downField("subjectType"))
        .as[Option[SubjectType]]
      sectorIdentifierUri <- c
        .downField("sectorIdentifierUri")
        .as[Option[String]]
      derivedSectorIdentifier <- c
        .downField("derivedSectorIdentifier")
        .as[Option[String]]
      jwksUri <- c.downField("jwksUri").as[Option[String]]
      jwks <- c.downField("jwks").as[Option[String]]
      userInfoSignAlg <- mapEmptyStringToNull(c.downField("userInfoSignAlg"))
        .as[Option[JwsAlg]]
      userInfoEncryptionAlg <-
        mapEmptyStringToNull(c.downField("userInfoEncryptionAlg"))
          .as[Option[JweAlg]]
      userInfoEncryptionEnc <-
        mapEmptyStringToNull(c.downField("userInfoEncryptionEnc"))
          .as[Option[JweEnc]]
      ...
      bcNotificationEndpoint <- c
        .downField("bcNotificationEndpoint")
        .as[Option[String]]
      bcRequestSignAlg <- mapEmptyStringToNull(c.downField("bcRequestSignAlg"))
        .as[Option[JwsAlg]]
      bcUserCodeRequired <- c
        .downField("bcUserCodeRequired")
        .as[Option[Boolean]]
      attributes <- c.downField("attributes").as[Option[Seq[Pair]]]
      extension <- c.downField("extension").as[Option[ClientExtension]]
      authorizationDetailsTypes <- c
        .downField("authorizationDetailsTypes")
      ...
      automaticallyRegistered <- c
        .downField("automaticallyRegistered")
        .as[Option[Boolean]]
      explicitlyRegistered <- c
        .downField("explicitlyRegistered")
        .as[Option[Boolean]]
      rsRequestSigned <- c.downField("rsRequestSigned").as[Option[Boolean]]
      rsSignedRequestKeyId <- c
        .downField("rsSignedRequestKeyId")
        .as[Option[String]]
      clientRegistrationTypes <-
        c.downField("clientRegistrationTypes")
