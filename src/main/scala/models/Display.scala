...
/** The display mode which the client application requests by `display` request
  ...
  * parameter, `PAGE` is set as the default value. It is ensured that the value
  * of `display` is one of the supported display modes which are specified by
  * `supportedDisplays` configuration parameter of the service. If the display
  * mode specified by the authorization request is not supported, an error is
  * raised. Values for this property correspond to the values listed in
...
object Display {
  ...
  def withValueOpt(value: String): Option[Display] =
    Display.values.find(_.value == value)

  def withValue(value: String): Display =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"Display enum case not found: $value"
      )
    )

  given decoderDisplay: Decoder[Display] = Decoder.decodeString.map(withValue)
  given encoderDisplay: Encoder[Display] =
    Encoder.encodeString.contramap[Display](_.value)
