...
/** @param resultCode
  ...
  *   backchannel authentication request is the client ID alias. `false` if the
  *   value is the original numeric client ID.
  * @param clientName
  *   The name of the client application which has made the backchannel
  *   authentication request.
  * @param scopes
  *   The scopes requested by the backchannel authentication request. Basically,
  *   this property holds the value of the `scope` request parameter in the
  *   backchannel authentication request. However, because unregistered scopes
  *   are dropped on Authlete side, if the `scope` request parameter contains
  *   unknown scopes, the list returned by this property becomes different from
  *   the value of the `scope` request parameter. Note that `description`
  *   property and `descriptions` property of each `scope` object in the array
  *   contained in this property is always null even if descriptions of the
  *   scopes are registered.
  * @param claimNames
  *   The names of the claims which were requested indirectly via some special
  *   scopes. See [5.4. Requesting Claims using Scope
  ...
  *   request. Basically, this property holds the value of the `acr_values`
  ...
  *   `acr_values` request parameter contains unrecognized ACR values, the list
  *   returned by this property becomes different from the value of the
  ...
  * @param sub
  *   The value of the `sub` claim contained in the ID token hint included in
  ...
  *   the `backchannel_user_code_parameter` metadata of the client (= Client's
  *   `bcUserCodeRequired` property) and the
  ...
  *   The request context of the backchannel authentication request. It is the
  *   value of the request_context claim in the signed authentication request
  *   and its format is JSON. request_context is a new claim added by the
  *   FAPI-CIBA profile.
  * @param warnings
...
object BackchannelAuthenticationResponse {
  ...
  given encoderBackchannelAuthenticationResponse
      : Encoder[BackchannelAuthenticationResponse] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
          ...
          t.clientNotificationToken.map(v =>
            "clientNotificationToken" -> v.asJson
          ),
  ...
  given decoderBackchannelAuthenticationResponse
    ...
    Decoder.instance { c =>
      for {
        ...
        clientNotificationToken <- c
          .downField("clientNotificationToken")
          .as[Option[String]]
        ...
        deliveryMode <- mapEmptyStringToNull(c.downField("deliveryMode"))
          .as[Option[DeliveryMode]]
