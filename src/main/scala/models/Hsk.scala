...
/** Holds information about a key managed in an HSM (Hardware Security Module)
  ...
  * @param use
  *   Get the use of the key on the HSM. When the key use is \"sig\"
  *   (signature), the private key on the HSM is used to sign data and the
  *   corresponding public key is used to verify the signature. When the key use
  *   is \"enc\" (encryption), the private key on the HSM is used to decrypt
  *   encrypted data which have been encrypted with the corresponding public key
  * @param kid
...
object Hsk {
  ...
  given jsonCodec: JsonValueCodec[Hsk] = JsonCodecMaker.make(codecMakerConfig)

  given encoderHsk: Encoder[Hsk] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
  ...
  given decoderHsk: Decoder[Hsk] = Decoder.instance { c =>
    for {
