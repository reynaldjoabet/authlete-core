...
/** @param ticket
  ...
  *   JSON format. See [OpenID Connect Core 1.0, 5.1. Standard
  ...
  *   code.
  * @param scopes
  *   Scopes to associate with an access token and/or an authorization code. If
  *   a non-empty string array is given, it replaces the scopes specified by the
  *   original authorization request.
  * @param sub
  *   The value of the `sub` claim to embed in an ID token. If this request
  *   parameter is `null` or empty, the value of the `subject` request parameter
  *   is used as the value of the `sub` claim.
  * @param idtHeaderParams
  *   JSON that represents additional JWS header parameters for ID tokens that
  *   may be issued based on the authorization request.
  * @param claimsForTx
...
object AuthorizationIssueRequest {
  ...
  given jsonCodec: JsonValueCodec[AuthorizationIssueRequest] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderAuthorizationIssueRequest: Encoder[AuthorizationIssueRequest] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
  ...
  given decoderAuthorizationIssueRequest: Decoder[AuthorizationIssueRequest] =
    Decoder.instance { c =>
      for {
