...
object ClientType {
  ...
  def withValueOpt(value: String): Option[ClientType] =
    ClientType.values.find(_.value == value)

  def withValue(value: String): ClientType =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"ClientType enum case not found: $value"
      )
    )

  given decoderClientType: Decoder[ClientType] =
    Decoder.decodeString.map(withValue)
  given encoderClientType: Encoder[ClientType] =
    Encoder.encodeString.contramap[ClientType](_.value)
