...
/** The verified claims validation schema set.
  */
enum VerifiedClaimsValidationSchema(val value: String)
    derives ConfiguredJsonValueCodec {

  case Standard extends VerifiedClaimsValidationSchema("standard")
  case StandardidDocument
      extends VerifiedClaimsValidationSchema("standard+id_document")
    ...
    withValueOpt(value).getOrElse(
      throw java.lang
        .IllegalArgumentException(
          s"VerifiedClaimsValidationSchema enum case not found: $value"
        )
    )

  given decoderVerifiedClaimsValidationSchema
      : Decoder[VerifiedClaimsValidationSchema] = Decoder.decodeString
    .map(withValue)

  given encoderVerifiedClaimsValidationSchema
      : Encoder[VerifiedClaimsValidationSchema] = Encoder.encodeString
