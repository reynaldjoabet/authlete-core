...
/** @param resultCode
  ...
  * @param action
  * @param clientId
  *   The client ID of the client application to which the user code has been
  *   issued.
  * @param clientIdAlias
  ...
  * @param scopes
  *   The scopes requested by the device authorization request. Note that
  *   `description` property and `descriptions` property of each scope object in
  *   the array contained in this property is always null even if descriptions
  *   of the scopes are registered.
  * @param claimNames
  ...
  *   in OpenID Connect Core 1.0 for details. This property is always `null` if
  *   the `scope` request parameter of the device authorization request does not
  *   include the `openid` scope even if special scopes (such as `profile`) are
  *   included in the request (unless the openid scope is included in the
  *   default set of scopes which is used when the `scope` request parameter is
  *   omitted).
  * @param acrs
  ...
  *   request parameter.
  * @param expiresAt
  *   Get the date in milliseconds since the Unix epoch (1970-01-01) at which
      ...
      for {
        ...
        action <-
          mapEmptyStringToNull(c.downField("action"))
            .as[Option[DeviceVerificationResponseAction]]
