...
object GrantType {
  ...
  def withValueOpt(value: String): Option[GrantType] =
    GrantType.values.find(_.value == value)

  def withValue(value: String): GrantType =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"GrantType enum case not found: $value"
      )
    )

  given decoderGrantType: Decoder[GrantType] =
    Decoder.decodeString.map(withValue)
  given encoderGrantType: Encoder[GrantType] =
    Encoder.encodeString.contramap[GrantType](_.value)
