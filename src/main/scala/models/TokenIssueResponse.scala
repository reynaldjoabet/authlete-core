...
/** @param resultCode
  ...
  *   the client application. Its format is JSON.
  * @param accessToken
  *   The newly issued access token. This parameter is a non-null value only
  *   when the value of `action` parameter is `OK`.
  ...
  * @param refreshToken
  *   The refresh token. This parameter is a non-null value only when `action`
  *   is `OK` and the service supports the refresh token flow. If
  *   `refreshTokenKept` is set to `false`, a new refresh token is issued and
  *   the old refresh token used in the refresh token flow is invalidated. On
  *   the contrary, if `refreshTokenKept` is set to `true`, the refresh token
  ...
  *   The client ID.
  * @param clientIdAlias
  *   The client ID alias. If the client did not have an alias, this parameter
  *   is `null`.
  ...
  *   token has been issued by calling `/api/auth/token` API, this parameter is
  *   `null` if the flow of the token request was [Client Credentials
...
object TokenIssueResponse {
  ...
  given jsonCodec: JsonValueCodec[TokenIssueResponse] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderTokenIssueResponse: Encoder[TokenIssueResponse] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
  ...
  given decoderTokenIssueResponse: Decoder[TokenIssueResponse] =
    Decoder.instance { c =>
      for {
        ...
        action <- mapEmptyStringToNull(c.downField("action"))
          .as[Option[TokenIssueResponseAction]]
