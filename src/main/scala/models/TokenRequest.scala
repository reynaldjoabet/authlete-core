...
/** @param parameters
  *   OAuth 2.0 token request parameters which are the request parameters that
  *   the OAuth 2.0 token endpoint of the authorization server implementation
  *   received from the client application. The value of parameters is the
  *   entire entity body (which is formatted in
  *   `application/x-www-form-urlencoded`) of the request from the client
  *   application.
  * @param clientId
  *   The client ID extracted from `Authorization` header of the token request
  *   from the client application. If the token endpoint of the authorization
  *   server implementation supports basic authentication as a means of client
  *   authentication, and the request from the client application contained its
  *   client ID in `Authorization` header, the value should be extracted and set
  *   to this parameter.
  * @param clientSecret
  *   The client secret extracted from `Authorization` header of the token
  *   request from the client application. If the token endpoint of the
  ...
  *   for details.
  * @param dpop
  *   `DPoP` header presented by the client during the request to the token
  ...
  *   details.
  * @param htm
  *   HTTP method of the token request. This field is used to validate the
  *   `DPoP` header. In normal cases, the value is `POST`. When this parameter
  *   is omitted, `POST` is used as the default value. See [OAuth 2.0
  *   Demonstration of Proof-of-Possession at the Application Layer
  *   (DPoP)](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-dpop) for
  *   details.
  * @param htu
  *   URL of the token endpoint. This field is used to validate the `DPoP`
  *   header. If this parameter is omitted, the `tokenEndpoint` property of the
  *   Service is used as the default value. See [OAuth 2.0 Demonstration of
...
object TokenRequest {
  ...
  given jsonCodec: JsonValueCodec[TokenRequest] =
