...
object TokenResponseAction {
  ...
  def withValueOpt(value: String): Option[TokenResponseAction] =
    TokenResponseAction.values
      .find(_.value == value)

  def withValue(value: String): TokenResponseAction =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"TokenResponseAction enum case not found: $value"
      )
    )

  given decoderTokenResponseAction: Decoder[TokenResponseAction] =
    Decoder.decodeString
      .map(withValue)

  given encoderTokenResponseAction: Encoder[TokenResponseAction] =
