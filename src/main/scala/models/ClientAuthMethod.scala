...
object ClientAuthMethod {
  ...
  def withValueOpt(value: String): Option[ClientAuthMethod] =
  ...
  def withValue(value: String): ClientAuthMethod =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"ClientAuthMethod enum case not found: $value"
      )
    )

  given decoderClientAuthMethod: Decoder[ClientAuthMethod] =
    Decoder.decodeString.map(withValue)

  given encoderClientAuthMethod: Encoder[ClientAuthMethod] =
