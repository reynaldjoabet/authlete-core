...
/** @param resultCode
  ...
  *   device authorization request is the client ID alias. `false` if the value
  ...
  * @param scopes
  *   The scopes requested by the device authorization request. Basically, this
  *   property holds the value of the scope request parameter in the device
  *   authorization request. However, because unregistered scopes are dropped on
  *   Authlete side, if the `scope` request parameter contains unknown scopes,
  *   the list returned by this property becomes different from the value of the
  *   `scope` request parameter. Note that `description` property and
  *   `descriptions` property of each scope object in the array contained in
  *   this property is always `null` even if descriptions of the scopes are
  *   registered.
  * @param claimNames
  ...
  *   request parameter contains unrecognized ACR values, the list returned by
  *   this property becomes different from the value of the `acr_values` request
  *   parameter.
  * @param deviceCode
  ...
  *   code. This corresponds to the `verification_uri_complete` property in the
  *   response to the client.
  * @param expiresIn
  *   The duration of the device verification code in seconds. This corresponds
  *   to the `expires_in` property in the response to the client.
  * @param interval
...
object DeviceAuthorizationResponse {
  ...
  given encoderDeviceAuthorizationResponse
      : Encoder[DeviceAuthorizationResponse] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
          ...
          t.verificationUriComplete.map(v =>
            "verificationUriComplete" -> v.asJson
          ),
  ...
  given decoderDeviceAuthorizationResponse
      : Decoder[DeviceAuthorizationResponse] =
    Decoder.instance { c =>
      for {
        resultCode <- c.downField("resultCode").as[Option[String]]
        resultMessage <- c.downField("resultMessage").as[Option[String]]
        action <-
          mapEmptyStringToNull(c.downField("action"))
            .as[Option[DeviceAuthorizationResponseAction]]
        ...
        verificationUriComplete <- c
          .downField("verificationUriComplete")
          .as[Option[String]]
