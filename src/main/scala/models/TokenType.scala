...
object TokenType {
  ...
  def withValueOpt(value: String): Option[TokenType] =
    TokenType.values.find(_.value == value)

  def withValue(value: String): TokenType =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"TokenType enum case not found: $value"
      )
    )

  given decoderTokenType: Decoder[TokenType] =
    Decoder.decodeString.map(withValue)
  given encoderTokenType: Encoder[TokenType] =
    Encoder.encodeString.contramap[TokenType](_.value)
