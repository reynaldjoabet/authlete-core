...
/** @param kty
  ...
  * @param use
  *   The key on the HSM. When the key use is \"sig\" (signature), the private
  *   key on the HSM is used to sign data and the corresponding public key is
  *   used to verify the signature. When the key use is \"enc\" (encryption),
  *   the private key on the HSM is used to decrypt encrypted data which have
...
object HskCreateRequest {
  ...
  given jsonCodec: JsonValueCodec[HskCreateRequest] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderHskCreateRequest: Encoder[HskCreateRequest] = Encoder.instance {
    t =>
      Json.fromFields {
        Seq(
  ...
  given decoderHskCreateRequest: Decoder[HskCreateRequest] = Decoder.instance {
    c =>
      for {
