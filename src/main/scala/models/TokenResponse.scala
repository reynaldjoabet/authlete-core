...
/** @param resultCode
  ...
  *   application must specify username when it uses [Resource Owner Password
  *   Grant](https://datatracker.ietf.org/doc/html/rfc6749#section-4.3). In
  *   other words, when the value of `grant_type` request parameter is
  *   `password`, `username` request parameter must come along. This parameter
  *   has a value only if the value of `grant_type` request parameter is
  *   `password` and the token request is valid.
  * @param password
  *   The value of `password` request parameter in the token request. The client
  *   application must specify password when it uses [Resource Owner Password
  *   Grant](https://datatracker.ietf.org/doc/html/rfc6749#section-4.3). In
  *   other words, when the value of `grant_type` request parameter is
  *   `password`, `password` request parameter must come along. This parameter
  *   has a value only if the value of `grant_type` request parameter is
  *   `password` and the token request is valid.
  * @param ticket
  *   The ticket which is necessary to call Authlete's `/auth/token/fail` API or
  *   `/auth/token/issue` API. This parameter has a value only if the value of
  *   `grant_type` request parameter is `password` and the token request is
  *   valid.
  * @param accessToken
  ...
  *   The datetime at which the newly issued refresh token will expire. The
  *   value is represented in milliseconds since the Unix epoch (1970-01-01).
  * @param refreshTokenDuration
  *   The duration of the newly issued refresh token in seconds.
  * @param idToken
  *   The newly issued ID token. Note that an ID token is issued from a token
  *   endpoint only when the `response_type` request parameter of the
  *   authorization request to an authorization endpoint has contained `code`
  *   and the `scope` request parameter has contained `openid`.
  * @param grantType
  *   The grant type of the token request.
  * @param clientId
  *   The client ID.
  * @param clientIdAlias
  *   The client ID alias when the token request was made. If the client did not
  *   have an alias, this parameter is `null`. Also, if the token request was
  *   invalid and it failed to identify a client, this parameter is `null`.
  * @param clientIdAliasUsed
  *   The flag which indicates whether the client ID alias was used when the
  *   token request was made. `true` if the client ID alias was used when the
  *   token request was made.
  * @param subject
  *   The subject (= resource owner's ID) of the access token. Even if an access
  *   token has been issued by the call of `/api/auth/token` API, this parameter
  *   is `null` if the flow of the token request was [Client Credentials
  ...
  *   `null` when no extra property is associated with the issued access token.
  * @param jwtAccessToken
  *   The newly issued access token in JWT format. If the authorization server
  *   is configured to issue JWT-based access tokens (= if the service's
  *   `accessTokenSignAlg` value is a non-null value), a JWT-based access token
  *   is issued along with the original random-string one.
  * @param resources
  ...
  * @param assertion
  *   For RFC 7523 JSON Web Token (JWT) Profile for OAuth 2.0 Client
  *   Authentication and Authorization Grants
  * @param previousRefreshTokenUsed
  *   Indicate whether the previous refresh token that had been kept in the
  *   database for a short time was used
...
object TokenResponse {
  ...
  given jsonCodec: JsonValueCodec[TokenResponse] =
  ...
  given encoderTokenResponse: Encoder[TokenResponse] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
        ...
        t.previousRefreshTokenUsed.map(v =>
          "previousRefreshTokenUsed" -> v.asJson
        ),
