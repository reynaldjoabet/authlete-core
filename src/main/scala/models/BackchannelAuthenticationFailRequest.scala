...
/** @param ticket
  ...
  *   `/backchannel/authentication/fail` API. This request parameter is not
  *   mandatory but optional. If this request parameter is given and the ticket
  *   belongs to the service, the specified ticket is deleted from the database.
  *   Giving this parameter is recommended to clean up the storage area for the
  *   service.
  * @param reason
...
object BackchannelAuthenticationFailRequest {
  ...
  given encoderBackchannelAuthenticationFailRequest
  ...
  given decoderBackchannelAuthenticationFailRequest
      : Decoder[BackchannelAuthenticationFailRequest] =
    Decoder.instance { c =>
      for {
        ticket <- c.downField("ticket").as[String]
        reason <- c
          .downField("reason")
          .as[BackchannelAuthenticationFailRequestReason]
