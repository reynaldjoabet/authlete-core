...
object TokenRevokeRequest {
  ...
  given jsonCodec: JsonValueCodec[TokenRevokeRequest] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderTokenRevokeRequest: Encoder[TokenRevokeRequest] =
    Encoder.instance { t =>
      Json.fromFields {
        Seq(
          t.accessTokenIdentifier.map(v => "accessTokenIdentifier" -> v.asJson),
          t.refreshTokenIdentifier.map(v =>
            "refreshTokenIdentifier" -> v.asJson
          ),
  ...
  given decoderTokenRevokeRequest: Decoder[TokenRevokeRequest] =
    Decoder.instance { c =>
      for {
        accessTokenIdentifier <- c
          .downField("accessTokenIdentifier")
          .as[Option[String]]
        refreshTokenIdentifier <- c
          .downField("refreshTokenIdentifier")
          .as[Option[String]]
