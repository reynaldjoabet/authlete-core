...
object GrantManagementAction {
  ...
  def withValueOpt(value: String): Option[GrantManagementAction] =
    GrantManagementAction.values
      .find(_.value == value)

  def withValue(value: String): GrantManagementAction =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"GrantManagementAction enum case not found: $value"
      )
    )

  given decoderGrantManagementAction: Decoder[GrantManagementAction] =
    Decoder.decodeString
      .map(withValue)

  given encoderGrantManagementAction: Encoder[GrantManagementAction] =
