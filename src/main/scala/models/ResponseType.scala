...
object ResponseType {
  ...
  def withValueOpt(value: String): Option[ResponseType] =
    ResponseType.values.find(_.value == value)

  def withValue(value: String): ResponseType =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"ResponseType enum case not found: $value"
      )
    )

  given decoderResponseType: Decoder[ResponseType] =
    Decoder.decodeString.map(withValue)

  given encoderResponseType: Encoder[ResponseType] = Encoder.encodeString
