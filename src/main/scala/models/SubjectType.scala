...
/** The subject type that the client application requests. Details about the
  * subject type are described in [OpenID Connect Core 1.0, 8. Subjct Identifier
  * Types](https://openid.net/specs/openid-connect-core-1_0.html#SubjectIDTypes).
  * This property corresponds to `subject_type` in [OpenID Connect Dynamic
...
object SubjectType {
  ...
  def withValueOpt(value: String): Option[SubjectType] =
    SubjectType.values.find(_.value == value)

  def withValue(value: String): SubjectType =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"SubjectType enum case not found: $value"
      )
    )

  given decoderSubjectType: Decoder[SubjectType] =
    Decoder.decodeString.map(withValue)

  given encoderSubjectType: Encoder[SubjectType] = Encoder.encodeString
