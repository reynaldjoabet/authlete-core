...
object HskGetResponse {
  ...
  given jsonCodec: JsonValueCodec[HskGetResponse] =
    JsonCodecMaker.make(codecMakerConfig)

  given encoderHskGetResponse: Encoder[HskGetResponse] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
  ...
  given decoderHskGetResponse: Decoder[HskGetResponse] = Decoder.instance { c =>
    for {
      ...
      action <- mapEmptyStringToNull(c.downField("action"))
        .as[Option[HskCreateResponseAction]]
      hsk <- c.downField("hsk").as[Option[Hsk]]
    } yield HskGetResponse(
