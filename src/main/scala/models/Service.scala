...
/** @param number
  ...
  *   The sequential number of the service owner of the service. The value of
  *   this property is assigned by Authlete.
  * @param serviceName
  *   The name of this service.
  * @param issuer
  *   The issuer identifier of the service. A URL that starts with https:// and
  *   has no query or fragment component. The value of this property is used as
  *   `iss` claim in an [ID
  *   token](https://openid.net/specs/openid-connect-core-1_0.html#IDToken) and
  *   `issuer` property in the [OpenID Provider
  ...
  * @param apiSecret
  *   The API secret. A random 256-bit value encoded by base64url (43 letters).
  *   The value of this property is assigned by Authlete.
  * @param clientsPerDeveloper
  *   The maximum number of client applications that a developer is allowed to
  *   create. `0` means no limit.
  * @param clientIdAliasEnabled
  *   The flag to indicate whether the 'Client ID Alias' feature is enabled or
  *   not. When a new client is created, Authlete generates a numeric value and
  *   assigns it as a client ID to the newly created client. In addition to the
  *   client ID, each client can have a client ID alias. The client ID alias is,
  *   however, recognized only when this property (`clientIdAliasEnabled`) is
  *   set to `true`.
  * @param metadata
  *   The `metadata` of the service. The content of the returned array depends
  *   on contexts. The predefined service metadata is listed in the following
  *   table. | Key | Description | | --- | --- | | `clientCount` | The number of
  *   client applications which belong to this service. |
  * @param createdAt
  *   The time at which this service was created. The value is represented as
  *   milliseconds since the UNIX epoch (`1970-01-01`).
  * @param modifiedAt
  *   The time at which this service was last modified. The value is represented
  ...
  *   service side. The endpoint must be implemented if you do not implement the
  *   UI at the authorization endpoint but use the one provided by Authlete. The
  *   user authentication at the authorization endpoint provided by Authlete is
  ...
  *   If the value is not empty, Authlete generates Authorization header for
  *   Basic authentication when making a request to the authentication callback
  *   endpoint.
  ...
  *   endpoint provided by Authlete. You need to register a `client` application
  *   in each SNS that is set as this parameter and set Authlete server's
  *   `/api/sns/redirection` as the redirection endpoint of the client
  *   application.
  * @param snsCredentials
  *   `SNS` credentials which Authlete uses to make requests to SNSes. The
  *   format is JSON.
  * @param supportedAcrs
  *   Values of acrs (authentication context class references) that the service
  *   supports. The value of this property is used as `acr_values_supported`
  *   property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param developerAuthenticationCallbackEndpoint
  *   A Web API endpoint for developer authentication which is to be prepared on
  *   the server side. The endpoint must be implemented if you use Developer
  *   Console. The developer authentication at the login page of Developer
  *   Console is performed by making a `POST` request to this endpoint.
  * @param developerAuthenticationCallbackApiKey
  *   API key for basic authentication at the developer authentication callback
  *   endpoint. If the value is not empty, Authlete generates Authorization
  *   header for Basic authentication when making a request to the developer
  ...
  *   SNSes you want to support 'social login' in the login page of Developer
  *   Console provided by Authlete. You need to register a client application in
  *   each SNS checked here and set Authlete server's
  *   `/api/developer/sns/redirection` as the redirection endpoint of the client
  *   application.
  * @param developerSnsCredentials
  *   SNS credentials which Authlete uses to make requests to SNSes. The format
  *   is JSON.
  * @param supportedGrantTypes
  *   Values of `grant_type` request parameter that the service supports. The
  *   value of this property is used as `grant_types_supported property` in the
  *   [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param supportedResponseTypes
  *   Values of `response_type` request parameter that the service supports.
  *   Valid values are listed in Response Type. The value of this property is
  *   used as `response_types_supported` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  ...
  *   `authorization_details_types_supported` metadata. See \"OAuth 2.0 Rich
  *   Authorization Requests\" (RAR) for details.
  ...
  *   omitted. According to [RFC 6749](https://tools.ietf.org/html/rfc6749), an
  *   authorization server may include the `error_description` response
  *   parameter in error responses. If `true`, Authlete does not embed the
  *   `error_description` response parameter in error responses.
  * @param errorUriOmitted
  *   The flag to indicate whether the `error_uri` response parameter is
  *   omitted. According to [RFC 6749](https://tools.ietf.org/html/rfc6749), an
  *   authorization server may include the `error_uri` response parameter in
  *   error responses. If `true`, Authlete does not embed the `error_uri`
  *   response parameter in error responses.
  * @param authorizationEndpoint
  *   The authorization endpoint of the service. A URL that starts with
  *   `https://` and has no fragment component. For example,
  *   `https://example.com/auth/authorization`. The value of this property is
  *   used as `authorization_endpoint` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param directAuthorizationEndpointEnabled
  *   The flag to indicate whether the direct authorization endpoint is enabled
  *   or not. The path of the endpoint is
  *   `/api/auth/authorization/direct/service-api-key`.
  * @param supportedUiLocales
  *   UI locales that the service supports. Each element is a language tag
  *   defined in [RFC 5646](https://tools.ietf.org/html/rfc5646). For example,
  *   `en-US` and `ja-JP`. The value of this property is used as
  *   `ui_locales_supported` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param supportedDisplays
  *   Values of `display` request parameter that service supports. The value of
  *   this property is used as `display_values_supported` property in the
  *   Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param pkceRequired
  *   The flag to indicate whether the use of Proof Key for Code Exchange (PKCE)
  *   is always required for authorization requests by Authorization Code Flow.
  *   If `true`, `code_challenge` request parameter is always required for
  *   authorization requests using Authorization Code Flow. See [RFC
  *   7636](https://tools.ietf.org/html/rfc7636) (Proof Key for Code Exchange by
  *   OAuth Public Clients) for details about `code_challenge` request
  *   parameter.
  * @param pkceS256Required
  *   The flag to indicate whether `S256` is always required as the code
  *   challenge method whenever [PKCE (RFC
  *   7636)](https://tools.ietf.org/html/rfc7636) is used. If this flag is set
  *   to `true`, `code_challenge_method=S256` must be included in the
  *   authorization request whenever it includes the `code_challenge` request
  *   parameter. Neither omission of the `code_challenge_method` request
  *   parameter nor use of plain (`code_challenge_method=plain`) is allowed.
  * @param authorizationResponseDuration
  *   The duration of authorization response JWTs in seconds. [Financial-grade
  *   API: JWT Secured Authorization Response Mode for OAuth 2.0
  *   (JARM)](https://openid.net/specs/openid-financial-api-jarm.html) defines
  *   new values for the `response_mode` request parameter. They are
  ...
  *   endpoint will be packed into a JWT. This property is used to compute the
  *   value of the `exp` claim of the JWT.
  * @param tokenEndpoint
  *   The [token endpoint](https://tools.ietf.org/html/rfc6749#section-3.2) of
  *   the service. A URL that starts with `https://` and has not fragment
  *   component. For example, `https://example.com/auth/token`. The value of
  *   this property is used as `token_endpoint` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param directTokenEndpointEnabled
  *   The flag to indicate whether the direct token endpoint is enabled or not.
  *   The path of the endpoint is `/api/auth/token/direct/service-api-key`.
  * @param supportedTokenAuthMethods
  *   Client authentication methods supported by the token endpoint of the
  *   service. The value of this property is used as
  *   `token_endpoint_auth_methods_supports` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param missingClientIdAllowed
  *   The flag to indicate token requests from public clients without the
  *   `client_id` request parameter are allowed when the client can be guessed
  *   from `authorization_code` or `refresh_token`. This flag should not be set
  *   unless you have special reasons.
  * @param revocationEndpoint
  *   The [revocation endpoint](https://tools.ietf.org/html/rfc7009) of the
  *   service. A URL that starts with `https://`. For example,
  *   `https://example.com/auth/revocation`.
  * @param directRevocationEndpointEnabled
  *   The flag to indicate whether the direct revocation endpoint is enabled or
  *   not. The URL of the endpoint is
  ...
  *   The flag to indicate whether the direct userinfo endpoint is enabled or
  *   not. The path of the endpoint is
  ...
  *   The URI of the pushed authorization request endpoint. This property
  *   corresponds to the `pushed_authorization_request_endpoint` metadata
  *   defined in \"[5. Authorization Server
  ...
  *   The duration of pushed authorization requests in seconds. [OAuth 2.0
  *   Pushed Authorization
  *   Requests](https://tools.ietf.org/html/draft-lodderstedt-oauth-par) defines
  *   an endpoint (called \"pushed authorization request endpoint\") which
  ...
  *   represent the registered authorization requests. The client applications
  *   can use the URIs as the value of the `request_uri` request parameter in an
  *   authorization request. The property represents the duration of registered
  ...
  * @param parRequired
  *   The flag to indicate whether this service requires that clients use the
  *   pushed authorization request endpoint. This property corresponds to the
  *   `require_pushed_authorization_requests` server metadata defined in [OAuth
  *   2.0 Pushed Authorization
  *   Requests](https://tools.ietf.org/html/draft-lodderstedt-oauth-par).
  * @param requestObjectRequired
  *   The flag to indicate whether this service requires that authorization
  *   requests always utilize a request object by using either request or
  *   `request_uri` request parameter. If this flag is set to `true` and the
  *   value of `traditionalRequestObjectProcessingApplied` is `false`, the value
  ...
  *   1.0](https://openid.net/specs/openid-connect-core-1_0.html) or JAR (JWT
  *   Secured Authorization Request). Differences between rules in OpenID
  *   Connect Core 1.0 and ones in JAR are as follows. - JAR requires that a
  *   request object be always -signed. - JAR does not allow request parameters
  *   outside a request object to be referred to. - OIDC Core 1.0 requires that
  *   response_type request parameter exist outside a request object even if the
  *   request object includes the request parameter. - OIDC Core 1.0 requires
  *   that scope request parameter exist outside a request object if the
  *   authorization request is an - OIDC request even if the request object
  *   includes the request parameter. If this flag is set to `false` and the
  *   value of `requestObjectRequired` is `true`, the value of
  *   `require_signed_request_object` server metadata of this service is
  *   reported as `true` in the discovery document. The metadata is defined in
  *   JAR (JWT Secured Authorization Request). That
  *   `require_signed_request_object` is `true` means that authorization
  *   requests which don't conform to the JAR specification are rejected.
  * @param mutualTlsValidatePkiCertChain
  *   The flag to indicate whether this service validates certificate chains
  *   during PKI-based client mutual TLS authentication.
  ...
  *   The MTLS endpoint aliases. This property corresponds to the
  *   mtls_endpoint_aliases metadata defined in \"5. Metadata for Mutual TLS
  *   Endpoint Aliases\" of [OAuth 2.0 Mutual TLS Client Authentication and
  *   Certificate-Bound Access
  *   Tokens](https://datatracker.ietf.org/doc/rfc8705/). The aliases will be
  *   embedded in the response from the discovery endpoint like the following. ```json {   ......,   \"mtls_endpoint_aliases\": {     \"token_endpoint\":         \"https://mtls.example.com/token\",     \"revocation_endpoint\":    \"https://mtls.example.com/revo\",     \"introspection_endpoint\": \"https://mtls.example.com/introspect\"   } } ```
  * @param accessTokenType
  *   The access token type. This value is used as the value of `token_type`
  *   property in access token responses. If this service complies with [RFC
  *   6750](https://tools.ietf.org/html/rfc6750), the value of this property
  *   should be `Bearer`. See [RFC 6749 (OAuth 2.0), 7.1. Access Token
  ...
  *   The duration of access tokens in seconds. This value is used as the value
  *   of `expires_in` property in access token responses. `expires_in` is
  *   defined [RFC 6749, 5.1. Successful
  *   Response](https://tools.ietf.org/html/rfc6749#section-5.1).
  * @param singleAccessTokenPerSubject
  *   The flag to indicate whether the number of access tokens per subject (and
  *   per client) is at most one or can be more. If `true`, an attempt to issue
  *   a new access token invalidates existing access tokens that are associated
  *   with the same subject and the same client. Note that, however, attempts by
  *   [Client Credentials Flow](https://tools.ietf.org/html/rfc6749#section-4.4)
  *   do not invalidate existing access tokens because access tokens issued by
  *   Client Credentials Flow are not associated with any end-user's subject.
  *   Also note that an attempt by [Refresh Token
  ...
  *   The key ID to identify a JWK used for signing access tokens. A JWK Set can
  *   be registered as a property of a service. A JWK Set can contain 0 or more
  *   JWKs. Authlete Server has to pick up one JWK for signing from the JWK Set
  *   when it generates a JWT-based access token. Authlete Server searches the
  *   registered JWK Set for a JWK which satisfies conditions for access token
  *   signature. If the number of JWK candidates which satisfy the conditions is
  *   1, there is no problem. On the other hand, if there exist multiple
  *   candidates, a Key ID is needed to be specified so that Authlete Server can
  *   pick up one JWK from among the JWK candidates.
  * @param refreshTokenDuration
  *   The duration of refresh tokens in seconds. The related specifications have
  *   no requirements on refresh token duration, but Authlete sets expiration
  *   for refresh tokens.
  * @param refreshTokenDurationKept
  *   The flag to indicate whether the remaining duration of the used refresh
  *   token is taken over to the newly issued refresh token.
  * @param refreshTokenDurationReset
  *   The flag which indicates whether duration of refresh tokens are reset when
  *   they are used even if the `refreshTokenKept` property of this service set
  *   to is `true` (= even if \"Refresh Token Continuous Use\" is \"Kept\").
  *   This flag has no effect when the `refreshTokenKept` property is set to
  *   `false`. In other words, if this service issues a new refresh token on
  *   every refresh token request, the refresh token will have fresh duration
  *   (unless `refreshTokenDurationKept` is set to `true`) and this
  *   `refreshTokenDurationReset` property is not referenced.
  * @param refreshTokenKept
  *   The flag to indicate whether a refresh token remains unchanged or gets
  *   renewed after its use. If `true`, a refresh token used to get a new access
  *   token remains valid after its use. Otherwise, if `false`, a refresh token
  *   is invalidated after its use and a new refresh token is issued. See [RFC
  *   6749 6. Refreshing an Access
  *   Token](https://tools.ietf.org/html/rfc6749#section-6), as to how to get a
  *   new access token using a refresh token.
  * @param supportedScopes
  *   Scopes supported by the service. Authlete strongly recommends that the
  *   service register at least the following scopes. | Name | Description | |
  ...
  *   parameter to have values other than code and token. | | profile | A
  ...
  *   for details. | | email | A permission to get information about `email` and
  *   `email_verified` from the user info endpoint. See [OpenID Connect Core
  *   1.0, 5.4. Requesting Claims using Scope
  *   Values](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims)
  *   for details. | | address | A permission to get information about address
  *   from the user info endpoint. See [OpenID Connect Core 1.0, 5.4. Requesting
  *   Claims using Scope
  *   Values](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims)
  *   and [5.1.1. Address
  *   Claim](https://openid.net/specs/openid-connect-core-1_0.html#AddressClaim)
  *   for details. | | phone | A permission to get information about
  *   `phone_number` and `phone_number_verified` from the user info endpoint.
  *   See [OpenID Connect Core 1.0, 5.4. Requesting Claims using Scope
  *   Values](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims)
  *   for details. | | offline_access | A permission to get information from the
  *   user info endpoint even when the end-user is not present. See [OpenID
  *   Connect Core 1.0, 11. Offline
  *   Access](https://openid.net/specs/openid-connect-core-1_0.html#OfflineAccess)
  *   for details. | The value of this property is used as `scopes_supported`
  *   property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param scopeRequired
  *   The flag to indicate whether requests that request no scope are rejected
  *   or not. When a request has no explicit `scope` parameter and the service's
  *   pre-defined default scope set is empty, the authorization server regards
  *   the request requests no scope. When this flag is set to `true`, requests
  *   that request no scope are rejected. The requirement below excerpted from
  *   [RFC 6749 Section 3.3](https://tools.ietf.org/html/rfc6749#section-3.3)
  ...
  *   as *\"the default scope set does not exist\"* and want to strictly conform
  *   to the requirement above, this flag has to be `true`.
  * @param idTokenDuration
  *   'The duration of [ID
  *   token](https://openid.net/specs/openid-connect-core-1_0.html#IDToken)s in
  *   seconds. This value is used to calculate the value of `exp` claim in an ID
  *   token.'
  * @param allowableClockSkew
  *   The allowable clock skew between the server and clients in seconds. The
  *   clock skew is taken into consideration when time-related claims in a JWT
  *   (e.g. `exp`, `iat`, `nbf`) are verified.
  * @param supportedClaimTypes
  *   Claim types supported by the service. Valid values are listed in Claim
  *   Type. Note that Authlete currently doesn't provide any API to help
  *   implementations for `AGGREGATED` and `DISTRIBUTED`. The value of this
  *   property is used as `claim_types_supported` property in the [OpenID
  *   Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param supportedClaimLocales
  *   Claim locales that the service supports. Each element is a language tag
  *   defined in [RFC 5646](https://tools.ietf.org/html/rfc5646). For example,
  *   `en-US` and `ja-JP`. See [OpenID Connect Core 1.0, 5.2. Languages and
  *   Scripts](https://openid.net/specs/openid-connect-core-1_0.html#ClaimsLanguagesAndScripts)
  *   for details. The value of this property is used as
  *   `claims_locales_supported` property in the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  ...
  *   should be supported. The following is the list of standard claims. - `sub` -
  *   `name` - `given_name` - `family_name` - `middle_name` - `nickname` -
  *   `preferred_username` - `profile` - `picture` - `website` - `email` -
  *   `email_verified` - `gender` - `birthdate` - `zoneinfo` - `locale` -
  *   `phone_number` - `phone_number_verified` - `address` - `updated_at` The
  *   value of this property is used as `claims_supported` property in the
  *   [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  *   The service may support its original claim names. See [OpenID Connect Core
  *   1.0, 5.1.2. Additional
  *   Claims](https://openid.net/specs/openid-connect-core-1_0.html#AdditionalClaims).
  * @param claimShortcutRestrictive
  *   The flag indicating whether claims specified by shortcut scopes (e.g.
  *   `profile`) are included in the issued ID token only when no access token
  *   is issued. To strictly conform to the description below excerpted from
  *   [OpenID Connect Core 1.0 Section
  *   5.4](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims),
  *   this flag has to be `true`. > The Claims requested by the profile, email,
  *   address, and phone scope values are returned from the UserInfo Endpoint,
  *   as described in Section 5.3.2, when a response_type value is used that
  ...
  *   Set](https://tools.ietf.org/html/rfc7517) document. For example,
  *   `http://example.com/auth/jwks`. Client applications accesses this URL (1)
  *   to get the public key of the service to validate the signature of an ID
  *   token issued by the service and (2) to get the public key of the service
  *   to encrypt an request object of the client application. See [OpenID
  *   Connect Core 1.0, 10. Signatures and
  *   Encryption](https://openid.net/specs/openid-connect-core-1_0.html#SigEnc)
  *   for details. The value of this property is used as `jwks_uri` property in
  *   the [OpenID Provider
  *   Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  ...
  *   Set](https://tools.ietf.org/html/rfc7517) document. If this property is
  *   not `null` in a `/service/create` request or a `/service/update` request,
  *   Authlete hosts the content in the database. This property must not be
  *   `null` and must contain pairs of public/private keys if the service wants
  *   to support asymmetric signatures for ID tokens and asymmetric encryption
  *   for request objects. See [OpenID Connect Core 1.0, 10. Signatures and
  *   Encryption](https://openid.net/specs/openid-connect-core-1_0.html#SigEnc)
  *   for details.
  * @param idTokenSignatureKeyId
  *   The key ID to identify a JWK used for ID token signature using an
  *   asymmetric key. A JWK Set can be registered as a property of a Service. A
  *   JWK Set can contain 0 or more JWKs (See [RFC
  *   7517](https://tools.ietf.org/html/rfc7517) for details about JWK).
  *   Authlete Server has to pick up one JWK for signature from the JWK Set when
  *   it generates an ID token and signature using an asymmetric key is
  *   required. Authlete Server searches the registered JWK Set for a JWK which
  *   satisfies conditions for ID token signature. If the number of JWK
  *   candidates which satisfy the conditions is 1, there is no problem. On the
  *   other hand, if there exist multiple candidates, a [Key
  *   ID](https://tools.ietf.org/html/rfc7517#section-4.5) is needed to be
  *   specified so that Authlete Server can pick up one JWK from among the JWK
  *   candidates. This `idTokenSignatureKeyId` property exists for the purpose
  *   described above. For key rotation (OpenID Connect Core 1.0, [10.1.1.
  *   Rotation of Asymmetric Signing
  ...
  *   7517](https://tools.ietf.org/html/rfc7517) for details about JWK).
  *   Authlete Server has to pick up one JWK for signature from the JWK Set when
  *   it is required to sign user info (which is returned from [userinfo
  ...
  *   for a JWK which satisfies conditions for user info signature. If the
  *   number of JWK candidates which satisfy the conditions is 1, there is no
  *   problem. On the other hand, if there exist multiple candidates, a [Key
  *   ID](https://tools.ietf.org/html/rfc7517#section-4.5) is needed to be
  *   specified so that Authlete Server can pick up one JWK from among the JWK
  *   candidates. This `userInfoSignatureKeyId` property exists for the purpose
  *   described above. For key rotation (OpenID Connect Core 1.0, [10.1.1.
  *   Rotation of Asymmetric Signing
  *   Keys](http://openid.net/specs/openid-connect-core-1_0.html#RotateSigKeys)),
  *   this mechanism is needed.
  ...
  *   using an asymmetric key. [Financial-grade API: JWT Secured Authorization
  ...
  *   specified as the response mode, response parameters from the authorization
  *   endpoint will be packed into a JWT. This property is used to compute the
  *   value of the `exp` claim of the JWT. Authlete Server searches the JWK Set
  *   for a JWK which satisfies conditions for authorization response signature.
  *   If the number of JWK candidates which satisfy the conditions is 1, there
  *   is no problem. On the other hand, if there exist multiple candidates, a
  *   Key ID is needed to be specified so that Authlete Server can pick up one
  *   JWK from among the JWK candidates. This property exists to specify the key
  *   ID.
  * @param userInfoEndpoint
  *   The [user info
  *   endpoint](http://openid.net/specs/openid-connect-core-1_0.html#UserInfo)
  *   of the service. A URL that starts with `https://`. For example,
  *   `https://example.com/auth/userinfo`. The value of this property is used as
  *   `userinfo_endpoint` property in the [OpenID Provider
  ...
  *   The boolean flag which indicates whether the [OAuth 2.0 Dynamic Client
  *   Registration Protocol](https://tools.ietf.org/html/rfc7591) is supported.
  * @param registrationEndpoint
  *   The [registration
  *   endpoint](http://openid.net/specs/openid-connect-registration-1_0.html#ClientRegistration)
  *   of the service. A URL that starts with `https://`. For example,
  *   `https://example.com/auth/registration`. The value of this property is
  *   used as `registration_endpoint` property in the [OpenID Provider
  *   Metadata](http://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  ...
  *   basis of the client's management endpoint by appending `/clientid}/` to it
  *   as a path element. If this is unset, the value of `registrationEndpoint`
  *   will be used as the URI base instead.
  * @param policyUri
  *   The URL of the \"Policy\" of the service. The value of this property is
  *   used as `op_policy_uri` property in the [OpenID Provider
  *   Metadata](http://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param tosUri
  *   The URL of the \"Terms Of Service\" of the service. The value of this
  *   property is used as `op_tos_uri` property in the [OpenID Provider
  *   Metadata](http://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  * @param serviceDocumentation
  *   The URL of a page where documents for developers can be found. The value
  *   of this property is used as `service_documentation` property in the
  *   [OpenID Provider
  *   Metadata](http://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
  ...
  *   The supported backchannel token delivery modes. This property corresponds
  *   to the `backchannel_token_delivery_modes_supported` metadata. Backchannel
  *   token delivery modes are defined in the specification of [CIBA (Client
  ...
  *   backchannel authentication endpoint in seconds. This is used as the value
  *   of the `expires_in` property in responses from the backchannel
  *   authentication endpoint.
  * @param backchannelPollingInterval
  *   The minimum interval between polling requests to the token endpoint from
  *   client applications in seconds. This is used as the value of the
  *   `interval` property in responses from the backchannel authentication
  *   endpoint.
  * @param backchannelUserCodeParameterSupported
  *   The boolean flag which indicates whether the `user_code` request parameter
  ...
  *   The flag to indicate whether the `binding_message` request parameter is
  *   always required whenever a backchannel authentication request is judged as
  *   a request for Financial-grade API. The FAPI-CIBA profile requires that the
  *   authorization server _\"shall ensure unique authorization context exists
  *   in the authorization request or require a `binding_message` in the
  *   authorization request\"_ (FAPI-CIBA, 5.2.2, 2). The simplest way to
  *   fulfill this requirement is to set this property to `true`. If this
  *   property is set to `false`, the `binding_message` request parameter
  ...
  *   The URI of the device authorization endpoint. Device authorization
  *   endpoint is defined in the specification of OAuth 2.0 Device Authorization
  *   Grant.
  * @param deviceVerificationUri
  *   The verification URI for the device flow. This URI is used as the value of
  *   the `verification_uri` parameter in responses from the device
  *   authorization endpoint.
  * @param deviceVerificationUriComplete
  *   The verification URI for the device flow with a placeholder for a user
  *   code. This URI is used to build the value of the
  *   `verification_uri_complete` parameter in responses from the device
  *   authorization endpoint. It is expected that the URI contains a fixed
  *   string `USER_CODE` somewhere as a placeholder for a user code. For
  *   example, like the following.
  *   `https://example.com/device?user\\_code=USER\\_CODE` The fixed string is
  *   replaced with an actual user code when Authlete builds a verification URI
  *   with a user code for the `verification_uri_complete` parameter. If this
  *   URI is not set, the `verification_uri_complete` parameter won't appear in
  *   device authorization responses.
  * @param deviceFlowCodeDuration
  *   The duration of device verification codes and end-user verification codes
  *   issued from the device authorization endpoint in seconds. This is used as
  *   the value of the `expires_in` property in responses from the device
  *   authorization endpoint.
  * @param deviceFlowPollingInterval
  *   The minimum interval between polling requests to the token endpoint from
  *   client applications in seconds in device flow. This is used as the value
  *   of the `interval` property in responses from the device authorization
  *   endpoint.
  ...
  *   optional even when the authorization request is regarded as a FAPI-Part2
  *   request. The final version of Financial-grade API was approved in January,
  *   2021. The Part 2 of the final version has new requirements on lifetime of
  *   request objects. They require that request objects contain an `nbf` claim
  *   and the lifetime computed by `exp` - `nbf` be no longer than 60 minutes.
  *   Therefore, when an authorization request is regarded as a FAPI-Part2
  *   request, the request object used in the authorization request must contain
  *   an nbf claim. Otherwise, the authorization server rejects the
  *   authorization request. When this flag is `true`, the `nbf` claim is
  *   treated as an optional claim even when the authorization request is
  *   regarded as a FAPI-Part2 request. That is, the authorization server does
  *   not perform the validation on lifetime of the request object. Skipping the
  *   validation is a violation of the FAPI specification. The reason why this
  *   flag has been prepared nevertheless is that the new requirements (which do
  *   not exist in the Implementer's Draft 2 released in October, 2018) have big
  *   impacts on deployed implementations of client applications and Authlete
  *   thinks there should be a mechanism whereby to make the migration from ID2
  *   to Final smooth without breaking live systems.
  * @param issSuppressed
  *   The flag indicating whether generation of the iss response parameter is
  *   suppressed. \"OAuth 2.0 Authorization Server Issuer Identifier in
  *   Authorization Response\" has defined a new authorization response
  *   parameter, `iss`, as a countermeasure for a certain type of mix-up
  *   attacks. The specification requires that the `iss` response parameter
  *   always be included in authorization responses unless JARM (JWT Secured
  *   Authorization Response Mode) is used. When this flag is `true`, the
  *   authorization server does not include the `iss` response parameter in
  *   authorization responses. By turning this flag on and off, developers of
  *   client applications can experiment the mix-up attack and the effect of the
  *   `iss` response parameter. Note that this flag should not be `true` in
  *   production environment unless there are special reasons for it.
  * @param supportedCustomClientMetadata
  *   custom client metadata supported by this service. Standard specifications
  *   define client metadata as necessary. The following are such examples. *
  *   [OpenID Connect Dynamic Client Registration
  *   1.0](https://openid.net/specs/openid-connect-registration-1_0.html) * [RFC
  *   7591 OAuth 2.0 Dynamic Client Registration
  *   Protocol](https://www.rfc-editor.org/rfc/rfc7591.html) * [RFC 8705 OAuth
  *   2.0 Mutual-TLS Client Authentication and Certificate-Bound Access
  *   Tokens](https://www.rfc-editor.org/rfc/rfc8705.html) * [OpenID Connect
  ...
  *   Request (JAR)](https://datatracker.ietf.org/doc/draft-ietf-oauth-jwsreq/)
  ...
  *   (PAR)](https://datatracker.ietf.org/doc/rfc9126/) * [OAuth 2.0 Rich
  *   Authorization Requests
  *   (RAR)](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar/) Standard
  *   client metadata included in Client Registration Request and Client Update
  *   Request (cf. [OIDC
  ...
  *   7592](https://www.rfc-editor.org/rfc/rfc7592.html)) are, if supported by
  *   Authlete, stored into Authlete database. On the other hand, unrecognized
  *   client metadata are discarded. By listing up custom client metadata in
  *   advance by using this property (`supportedCustomClientMetadata`), Authlete
  *   can recognize them and stores their values into the database. The stored
  *   custom client metadata values can be referenced by `customMetadata`.
  * @param tokenExpirationLinked
  *   The flag indicating whether the expiration date of an access token never
  *   exceeds that of the corresponding refresh token. When a new access token
  *   is issued by a refresh token request (= a token request with
  *   `grant_type=refresh_token`), the expiration date of the access token may
  *   exceed the expiration date of the corresponding refresh token. This
  *   behavior itself is not wrong and may happen when `refreshTokenKept` is
  *   `true` and/or when `refreshTokenDurationKept` is `true`. When this flag is
  *   `true`, the expiration date of an access token never exceeds that of the
  *   corresponding refresh token regardless of the calculated duration based on
  *   other settings such as `accessTokenDuration`, `accessTokenDuration` in
  *   `extension` and `access_token.duration` scope attribute. It is technically
  *   possible to set a value which is bigger than the duration of refresh
  *   tokens as the duration of access tokens although it is strange. In the
  *   case, the duration of an access token becomes longer than the duration of
  *   the refresh token which is issued together with the access token. Even if
  *   the duration values are configured so, if this flag is `true`, the
  *   expiration date of the access token does not exceed that of the refresh
  *   token. That is, the duration of the access token will be shortened, and as
  *   a result, the access token and the refresh token will have the same
  *   expiration date.
  * @param frontChannelRequestObjectEncryptionRequired
  *   The flag indicating whether encryption of request object is required when
  *   the request object is passed through the front channel. This flag does not
  *   affect the processing of request objects at the Pushed Authorization
  *   Request Endpoint, which is defined in [OAuth 2.0 Pushed Authorization
  *   Requests](https://datatracker.ietf.org/doc/rfc9126/). Unecrypted request
  *   objects are accepted at the endpoint even if this flag is `true`. This
  *   flag does not indicate whether a request object is always required. There
  *   is a different flag, `requestObjectRequired`, for the purpose. See the
  *   description of `requestObjectRequired` for details. Even if this flag is
  *   `false`, encryption of request object is required if the
  *   `frontChannelRequestObjectEncryptionRequired` flag of the client is
  *   `true`.
  * @param requestObjectEncryptionAlgMatchRequired
  *   The flag indicating whether the JWE alg of encrypted request object must
  *   match the `request_object_encryption_alg` client metadata of the client
  *   that has sent the request object. The request_object_encryption_alg client
  *   metadata itself is defined in [OpenID Connect Dynamic Client Registration
  *   1.0](https://openid.net/specs/openid-connect-registration-1_0.html) as
  *   follows. > request_object_encryption_alg > > OPTIONAL. JWE [JWE] alg
  *   algorithm [JWA] the RP is declaring that it may use for encrypting Request
  *   Objects sent to the OP. This parameter SHOULD be included when symmetric
  ...
  *   Nested JWT, as defined in [JWT]. The default, if omitted, is that the RP
  *   is not declaring whether it might encrypt any Request Objects. The point
  *   here is \"The RP MAY still use other supported encryption algorithms or
  ...
  *   details. Even if this flag is `false`, the match is required if the
  *   `requestObjectEncryptionAlgMatchRequired` flag of the client is `true`.
  * @param requestObjectEncryptionEncMatchRequired
  *   The flag indicating whether the JWE `enc` of encrypted request object must
  *   match the `request_object_encryption_enc` client metadata of the client
  *   that has sent the request object. The `request_object_encryption_enc`
  *   client metadata itself is defined in [OpenID Connect Dynamic Client
  *   Registration
  *   1.0](https://openid.net/specs/openid-connect-registration-1_0.html) as
  *   follows. > request_object_encryption_enc > > OPTIONAL. JWE enc algorithm
  *   [JWA] the RP is declaring that it may use for encrypting Request Objects
  *   sent to the OP. If request_object_encryption_alg is specified, the default
  *   for this value is A128CBC-HS256. When request_object_encryption_enc is
  *   included, request_object_encryption_alg MUST also be provided. The
  *   Client's property that represents the client metadata is
  ...
  *   `UNAUTHORIZED` as a value of the `action` response parameter when
  ...
  *   component is localhost, `127.0.0.1` or `::1`), the port number component
  *   is ignored when the specified redirection URI is compared to
  *   pre-registered ones. This behavior is described in [7.3. Loopback
  *   Interface Redirection](
  *   https://www.rfc-editor.org/rfc/rfc8252.html#section-7.3) of [RFC 8252
  ...
  *   of [RFC 6749](https://www.rfc-editor.org/rfc/rfc6749.html) states _\"If
  *   the client registration included the full redirection URI, the
  *   authorization server MUST compare the two URIs using simple string
  *   comparison as defined in [RFC3986] Section 6.2.1.\"_ Also, the description
  *   of `redirect_uri` in [3.1.2.1. Authentication
  *   Request](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest)
  *   of [OpenID Connect Core
  *   1.0](https://openid.net/specs/openid-connect-core-1_0.html) states _\"This
  ...
  *   described in Section 6.2.1 of [RFC3986] (**Simple String Comparison**).\"_
  *   These \"Simple String Comparison\" requirements are preceded by this flag.
  *   That is, even when the conditions described in RFC 6749 and OpenID Connect
  *   Core 1.0 are satisfied, the port number component of loopback redirection
  *   URIs can be variable when this flag is `true`. [8.3. Loopback Redirect
  ...
  *   follows. > While redirect URIs using localhost (i.e.,
  ...
  *   However, Authlete allows the port number component to be variable in the
  *   case of `localhost`, too. It is left to client applications whether they
  *   use `localhost` or a literal loopback IP address (`127.0.0.1` for IPv4 or
  *   `::1` for IPv6). Section 7.3 and Section 8.3 of [RFC
  *   8252](https://www.rfc-editor.org/rfc/rfc8252.html) state that loopback
  *   redirection URIs use the `\"http\"` scheme, but Authlete allows the port
  *   number component to be variable in other cases (e.g. in the case of the
  *   `\"https\"` scheme), too.
  ...
  *   {@code attachments_supported} server metadata which was added by the third
...
case class Service(
    ...
    supportedClientRegistrationTypes: Option[Seq[ClientRegistrationType]] =
      None,
...
object Service {
  ...
  given jsonCodec: JsonValueCodec[Service] =
  ...
  given encoderService: Encoder[Service] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
        ...
        t.authenticationCallbackEndpoint.map(v =>
          "authenticationCallbackEndpoint" -> v.asJson
        ),
        t.authenticationCallbackApiKey.map(v =>
          "authenticationCallbackApiKey" -> v.asJson
        ),
        t.authenticationCallbackApiSecret.map(v =>
          "authenticationCallbackApiSecret" -> v.asJson
        ),
        ...
        t.supportedDeveloperSnses.map(v =>
          "supportedDeveloperSnses" -> v.asJson
        ),
        t.developerSnsCredentials.map(v =>
          "developerSnsCredentials" -> v.asJson
        ),
        ...
        t.supportedServiceProfiles.map(v =>
          "supportedServiceProfiles" -> v.asJson
        ),
        t.errorDescriptionOmitted.map(v =>
          "errorDescriptionOmitted" -> v.asJson
        ),
        ...
        t.authorizationResponseDuration.map(v =>
          "authorizationResponseDuration" -> v.asJson
        ),
        t.tokenEndpoint.map(v => "tokenEndpoint" -> v.asJson),
        t.directTokenEndpointEnabled.map(v =>
          "directTokenEndpointEnabled" -> v.asJson
        ),
        t.supportedTokenAuthMethods.map(v =>
          "supportedTokenAuthMethods" -> v.asJson
        ),
        t.missingClientIdAllowed.map(v => "missingClientIdAllowed" -> v.asJson),
        t.revocationEndpoint.map(v => "revocationEndpoint" -> v.asJson),
        t.directRevocationEndpointEnabled.map(v =>
          "directRevocationEndpointEnabled" -> v.asJson
        ),
        t.supportedRevocationAuthMethods.map(v =>
          "supportedRevocationAuthMethods" -> v.asJson
        ),
        ...
        t.mutualTlsValidatePkiCertChain.map(v =>
          "mutualTlsValidatePkiCertChain" -> v.asJson
        ),
        t.trustedRootCertificates.map(v =>
          "trustedRootCertificates" -> v.asJson
        ),
        ...
        t.singleAccessTokenPerSubject.map(v =>
          "singleAccessTokenPerSubject" -> v.asJson
        ),
        t.accessTokenSignAlg.map(v => "accessTokenSignAlg" -> v.asJson),
        t.accessTokenSignatureKeyId.map(v =>
          "accessTokenSignatureKeyId" -> v.asJson
        ),
        t.refreshTokenDuration.map(v => "refreshTokenDuration" -> v.asJson),
        t.refreshTokenDurationKept.map(v =>
          "refreshTokenDurationKept" -> v.asJson
        ),
        t.refreshTokenDurationReset.map(v =>
          "refreshTokenDurationReset" -> v.asJson
        ),
        ...
        t.claimShortcutRestrictive.map(v =>
          "claimShortcutRestrictive" -> v.asJson
        ),
        t.jwksUri.map(v => "jwksUri" -> v.asJson),
        t.directJwksEndpointEnabled.map(v =>
          "directJwksEndpointEnabled" -> v.asJson
        ),
        ...
        t.authorizationSignatureKeyId.map(v =>
          "authorizationSignatureKeyId" -> v.asJson
        ),
        t.userInfoEndpoint.map(v => "userInfoEndpoint" -> v.asJson),
        t.directUserInfoEndpointEnabled.map(v =>
          "directUserInfoEndpointEnabled" -> v.asJson
        ),
        t.dynamicRegistrationSupported.map(v =>
          "dynamicRegistrationSupported" -> v.asJson
        ),
        t.registrationEndpoint.map(v => "registrationEndpoint" -> v.asJson),
        t.registrationManagementEndpoint.map(v =>
          "registrationManagementEndpoint" -> v.asJson
        ),
        ...
        t.backchannelAuthReqIdDuration.map(v =>
          "backchannelAuthReqIdDuration" -> v.asJson
        ),
        t.backchannelPollingInterval.map(v =>
          "backchannelPollingInterval" -> v.asJson
        ),
        ...
        t.deviceAuthorizationEndpoint.map(v =>
          "deviceAuthorizationEndpoint" -> v.asJson
        ),
        t.deviceVerificationUri.map(v => "deviceVerificationUri" -> v.asJson),
        t.deviceVerificationUriComplete.map(v =>
          "deviceVerificationUriComplete" -> v.asJson
        ),
        t.deviceFlowCodeDuration.map(v => "deviceFlowCodeDuration" -> v.asJson),
        t.deviceFlowPollingInterval.map(v =>
          "deviceFlowPollingInterval" -> v.asJson
        ),
        t.userCodeCharset.map(v => "userCodeCharset" -> v.asJson),
        t.userCodeLength.map(v => "userCodeLength" -> v.asJson),
        t.supportedTrustFrameworks.map(v =>
          "supportedTrustFrameworks" -> v.asJson
        ),
        t.supportedEvidence.map(v => "supportedEvidence" -> v.asJson),
        t.supportedIdentityDocuments.map(v =>
          "supportedIdentityDocuments" -> v.asJson
        ),
        t.supportedVerificationMethods.map(v =>
          "supportedVerificationMethods" -> v.asJson
        ),
        t.supportedVerifiedClaims.map(v =>
          "supportedVerifiedClaims" -> v.asJson
        ),
        ...
        t.supportedCustomClientMetadata.map(v =>
          "supportedCustomClientMetadata" -> v.asJson
        ),
        ...
        t.grantManagementEndpoint.map(v =>
          "grantManagementEndpoint" -> v.asJson
        ),
        t.grantManagementActionRequired.map(v =>
          "grantManagementActionRequired" -> v.asJson
        ),
        t.unauthorizedOnClientConfigSupported
          .map(v => "unauthorizedOnClientConfigSupported" -> v.asJson),
        t.dcrScopeUsedAsRequestable.map(v =>
          "dcrScopeUsedAsRequestable" -> v.asJson
        ),
        t.endSessionEndpoint.map(v => "endSessionEndpoint" -> v.asJson),
        t.loopbackRedirectionUriVariable.map(v =>
          "loopbackRedirectionUriVariable" -> v.asJson
        ),
        t.requestObjectAudienceChecked.map(v =>
          "requestObjectAudienceChecked" -> v.asJson
        ),
        ...
        t.federationSignatureKeyId.map(v =>
          "federationSignatureKeyId" -> v.asJson
        ),
        t.federationConfigurationDuration.map(v =>
          "federationConfigurationDuration" -> v.asJson
        ),
        t.federationRegistrationEndpoint.map(v =>
          "federationRegistrationEndpoint" -> v.asJson
        ),
        t.organizationName.map(v => "organizationName" -> v.asJson),
        t.predefinedTransformedClaims.map(v =>
          "predefinedTransformedClaims" -> v.asJson
        ),
        ...
        t.supportedDigestAlgorithms.map(v =>
          "supportedDigestAlgorithms" -> v.asJson
        ),
        t.supportedDocuments.map(v => "supportedDocuments" -> v.asJson),
        t.supportedDocumentsMethods.map(v =>
          "supportedDocumentsMethods" -> v.asJson
        ),
        ...
        t.supportedElectronicRecords.map(v =>
          "supportedElectronicRecords" -> v.asJson
        ),
        t.supportedClientRegistrationTypes.map(v =>
          "supportedClientRegistrationTypes" -> v.asJson
        ),
        ...
        t.tokenExchangeUnsignedJwtRejected.map(v =>
          "tokenExchangeUnsignedJwtRejected" -> v.asJson
        ),
        t.jwtGrantByIdentifiableClientsOnly
          .map(v => "jwtGrantByIdentifiableClientsOnly" -> v.asJson),
        t.jwtGrantEncryptedJwtRejected.map(v =>
          "jwtGrantEncryptedJwtRejected" -> v.asJson
        ),
        t.jwtGrantUnsignedJwtRejected.map(v =>
          "jwtGrantUnsignedJwtRejected" -> v.asJson
        ),
        t.dcrDuplicateSoftwareIdBlocked.map(v =>
          "dcrDuplicateSoftwareIdBlocked" -> v.asJson
        ),
        ...
        t.supportedDocumentsCheckMethods.map(v =>
          "supportedDocumentsCheckMethods" -> v.asJson
        ),
  ...
  given decoderService: Decoder[Service] = Decoder.instance { c =>
    for {
      ...
      clientIdAliasEnabled <- c
        .downField("clientIdAliasEnabled")
        .as[Option[Boolean]]
      ...
      authenticationCallbackApiKey <- c
        .downField("authenticationCallbackApiKey")
        .as[Option[String]]
      ...
      snsCredentials <- c
        .downField("snsCredentials")
        .as[Option[Seq[SnsCredentials]]]
      ...
      developerAuthenticationCallbackApiSecret <-
        c.downField("developerAuthenticationCallbackApiSecret")
          .as[Option[String]]
      supportedDeveloperSnses <- c
        .downField("supportedDeveloperSnses")
        .as[Option[Seq[Sns]]]
      developerSnsCredentials <-
        c.downField("developerSnsCredentials").as[Option[Seq[SnsCredentials]]]
      supportedGrantTypes <- c
        .downField("supportedGrantTypes")
        .as[Option[Seq[GrantType]]]
      supportedResponseTypes <- c
        .downField("supportedResponseTypes")
        .as[Option[Seq[ResponseType]]]
      supportedAuthorizationDetailsTypes <-
        c.downField("supportedAuthorizationDetailsTypes")
          .as[Option[Seq[String]]]
      supportedServiceProfiles <-
        c.downField("supportedServiceProfiles").as[Option[Seq[ServiceProfile]]]
      errorDescriptionOmitted <- c
        .downField("errorDescriptionOmitted")
        .as[Option[Boolean]]
      errorUriOmitted <- c.downField("errorUriOmitted").as[Option[Boolean]]
      authorizationEndpoint <- c
        .downField("authorizationEndpoint")
        .as[Option[URI]]
      directAuthorizationEndpointEnabled <-
        c.downField("directAuthorizationEndpointEnabled").as[Option[Boolean]]
      supportedUiLocales <- c
        .downField("supportedUiLocales")
        .as[Option[Seq[String]]]
      supportedDisplays <- c
        .downField("supportedDisplays")
        .as[Option[Seq[Display]]]
      pkceRequired <- c.downField("pkceRequired").as[Option[Boolean]]
      pkceS256Required <- c.downField("pkceS256Required").as[Option[Boolean]]
      authorizationResponseDuration <- c
        .downField("authorizationResponseDuration")
        .as[Option[Long]]
      tokenEndpoint <- c.downField("tokenEndpoint").as[Option[URI]]
      directTokenEndpointEnabled <- c
        .downField("directTokenEndpointEnabled")
        .as[Option[Boolean]]
      supportedTokenAuthMethods <-
        c.downField("supportedTokenAuthMethods")
          .as[Option[Seq[ClientAuthMethod]]]
      missingClientIdAllowed <- c
        .downField("missingClientIdAllowed")
        .as[Option[Boolean]]
      ...
      supportedRevocationAuthMethods <-
        c.downField("supportedRevocationAuthMethods")
          .as[Option[Seq[ClientAuthMethod]]]
      introspectionEndpoint <- c
        .downField("introspectionEndpoint")
        .as[Option[URI]]
      ...
      supportedIntrospectionAuthMethods <-
        c.downField("supportedIntrospectionAuthMethods")
          .as[Option[Seq[ClientAuthMethod]]]
      pushedAuthReqEndpoint <- c
        .downField("pushedAuthReqEndpoint")
        .as[Option[URI]]
      pushedAuthReqDuration <- c
        .downField("pushedAuthReqDuration")
        .as[Option[Long]]
      parRequired <- c.downField("parRequired").as[Option[Boolean]]
      requestObjectRequired <- c
        .downField("requestObjectRequired")
        .as[Option[Boolean]]
      traditionalRequestObjectProcessingApplied <-
        c.downField("traditionalRequestObjectProcessingApplied")
          .as[Option[Boolean]]
      mutualTlsValidatePkiCertChain <-
        c.downField("mutualTlsValidatePkiCertChain").as[Option[Boolean]]
      trustedRootCertificates <- c
        .downField("trustedRootCertificates")
        .as[Option[Seq[String]]]
      mtlsEndpointAliases <- c
        .downField("mtlsEndpointAliases")
        .as[Option[Seq[NamedUri]]]
      ...
      singleAccessTokenPerSubject <- c
        .downField("singleAccessTokenPerSubject")
        .as[Option[Boolean]]
      accessTokenSignAlg <-
        mapEmptyStringToNull(c.downField("accessTokenSignAlg"))
          .as[Option[JwsAlg]]
      accessTokenSignatureKeyId <- c
        .downField("accessTokenSignatureKeyId")
        .as[Option[String]]
      refreshTokenDuration <- c
        .downField("refreshTokenDuration")
        .as[Option[Long]]
      refreshTokenDurationKept <- c
        .downField("refreshTokenDurationKept")
        .as[Option[Boolean]]
      refreshTokenDurationReset <- c
        .downField("refreshTokenDurationReset")
        .as[Option[Boolean]]
      ...
      supportedClaimTypes <- c
        .downField("supportedClaimTypes")
        .as[Option[Seq[ClaimType]]]
      supportedClaimLocales <- c
        .downField("supportedClaimLocales")
        .as[Option[Seq[String]]]
      supportedClaims <- c.downField("supportedClaims").as[Option[Seq[String]]]
      claimShortcutRestrictive <- c
        .downField("claimShortcutRestrictive")
        .as[Option[Boolean]]
      jwksUri <- c.downField("jwksUri").as[Option[URI]]
      directJwksEndpointEnabled <- c
        .downField("directJwksEndpointEnabled")
        .as[Option[Boolean]]
      jwks <- c.downField("jwks").as[Option[String]]
      idTokenSignatureKeyId <- c
        .downField("idTokenSignatureKeyId")
        .as[Option[String]]
      userInfoSignatureKeyId <- c
        .downField("userInfoSignatureKeyId")
        .as[Option[String]]
      authorizationSignatureKeyId <- c
        .downField("authorizationSignatureKeyId")
        .as[Option[String]]
      ...
      registrationEndpoint <- c
        .downField("registrationEndpoint")
        .as[Option[URI]]
      ...
      serviceDocumentation <- c
        .downField("serviceDocumentation")
        .as[Option[URI]]
      backchannelAuthenticationEndpoint <-
        c.downField("backchannelAuthenticationEndpoint").as[Option[URI]]
      supportedBackchannelTokenDeliveryModes <-
        c.downField("supportedBackchannelTokenDeliveryModes")
          .as[Option[Seq[DeliveryMode]]]
      backchannelAuthReqIdDuration <- c
        .downField("backchannelAuthReqIdDuration")
        .as[Option[Int]]
      backchannelPollingInterval <- c
        .downField("backchannelPollingInterval")
        .as[Option[Int]]
      backchannelUserCodeParameterSupported <-
        c.downField("backchannelUserCodeParameterSupported").as[Option[Boolean]]
      backchannelBindingMessageRequiredInFapi <-
        c.downField("backchannelBindingMessageRequiredInFapi")
          .as[Option[Boolean]]
      deviceAuthorizationEndpoint <- c
        .downField("deviceAuthorizationEndpoint")
        .as[Option[URI]]
      deviceVerificationUri <- c
        .downField("deviceVerificationUri")
        .as[Option[URI]]
      deviceVerificationUriComplete <- c
        .downField("deviceVerificationUriComplete")
        .as[Option[URI]]
      deviceFlowCodeDuration <- c
        .downField("deviceFlowCodeDuration")
        .as[Option[Int]]
      deviceFlowPollingInterval <- c
        .downField("deviceFlowPollingInterval")
        .as[Option[Int]]
      userCodeCharset <-
        mapEmptyStringToNull(c.downField("userCodeCharset"))
          .as[Option[UserCodeCharset]]
      userCodeLength <- c.downField("userCodeLength").as[Option[Int]]
      supportedTrustFrameworks <- c
        .downField("supportedTrustFrameworks")
        .as[Option[Seq[String]]]
      supportedEvidence <- c
        .downField("supportedEvidence")
        .as[Option[Seq[String]]]
      ...
      supportedVerifiedClaims <- c
        .downField("supportedVerifiedClaims")
        .as[Option[Seq[String]]]
      ...
      tokenExpirationLinked <- c
        .downField("tokenExpirationLinked")
        .as[Option[Boolean]]
      frontChannelRequestObjectEncryptionRequired <-
        c.downField("frontChannelRequestObjectEncryptionRequired")
          .as[Option[Boolean]]
      requestObjectEncryptionAlgMatchRequired <-
        c.downField("requestObjectEncryptionAlgMatchRequired")
          .as[Option[Boolean]]
      requestObjectEncryptionEncMatchRequired <-
        c.downField("requestObjectEncryptionEncMatchRequired")
          .as[Option[Boolean]]
      hsmEnabled <- c.downField("hsmEnabled").as[Option[Boolean]]
      hsks <- c.downField("hsks").as[Option[Seq[Hsk]]]
      grantManagementEndpoint <- c
        .downField("grantManagementEndpoint")
        .as[Option[String]]
      ...
      dcrScopeUsedAsRequestable <- c
        .downField("dcrScopeUsedAsRequestable")
        .as[Option[Boolean]]
      ...
      accessTokenForExternalAttachmentEmbedded <-
        c.downField("accessTokenForExternalAttachmentEmbedded")
          .as[Option[Boolean]]
      ...
      federationSignatureKeyId <- c
        .downField("federationSignatureKeyId")
        .as[Option[String]]
      ...
      predefinedTransformedClaims <- c
        .downField("predefinedTransformedClaims")
        .as[Option[String]]
      refreshTokenIdempotent <- c
        .downField("refreshTokenIdempotent")
        .as[Option[Boolean]]
      signedJwksUri <- c.downField("signedJwksUri").as[Option[String]]
      supportedAttachments <- c
        .downField("supportedAttachments")
        .as[Option[Seq[AttachmentType]]]
      supportedDigestAlgorithms <- c
        .downField("supportedDigestAlgorithms")
        .as[Option[Seq[String]]]
      supportedDocuments <- c
        .downField("supportedDocuments")
        .as[Option[Seq[String]]]
      supportedDocumentsMethods <- c
        .downField("supportedDocumentsMethods")
        .as[Option[Seq[String]]]
      supportedDocumentsValidationMethods <-
        c.downField("supportedDocumentsValidationMethods")
          .as[Option[Seq[String]]]
      supportedDocumentsVerificationMethods <-
        c.downField("supportedDocumentsVerificationMethods")
          .as[Option[Seq[String]]]
      supportedElectronicRecords <-
        c.downField("supportedElectronicRecords").as[Option[Seq[String]]]
      supportedClientRegistrationTypes <-
        c.downField("supportedClientRegistrationTypes")
          .as[Option[Seq[ClientRegistrationType]]]
      tokenExchangeByIdentifiableClientsOnly <-
        c.downField("tokenExchangeByIdentifiableClientsOnly")
          .as[Option[Boolean]]
      tokenExchangeByConfidentialClientsOnly <-
        c.downField("tokenExchangeByConfidentialClientsOnly")
          .as[Option[Boolean]]
      ...
      jwtGrantUnsignedJwtRejected <- c
        .downField("jwtGrantUnsignedJwtRejected")
        .as[Option[Boolean]]
      ...
      openidDroppedOnRefreshWithoutOfflineAccess <-
        c.downField("openidDroppedOnRefreshWithoutOfflineAccess")
          .as[Option[Boolean]]
      ...
      resourceSignatureKeyId <- c
        .downField("resourceSignatureKeyId")
        .as[Option[String]]
