...
/** this is the 'alg' header value for encrypted JWT tokens. Depending upon the
  ...
  * the server. For instance: - as `authorizationEncryptionAlg` value, it refers
  * to the encoding algorithm used by server for transporting they keys on JARM
  * objects - as `requestEncryptionAlg` value, it refers to the expected key
  * transport encoding algorithm that server expect from client when encrypting
  * a Request Object - as `idTokenEncryptionAlg` value, it refers to the
  * algorithm used by the server to key transport of id_tokens **Please note
  * that some of the algorithms are more secure than others, some are not
  * supported very well cross platforms and some (like RSA1_5) is known to be
  * weak**.
  */
...
object JweAlg {
  ...
  def withValueOpt(value: String): Option[JweAlg] =
    JweAlg.values.find(_.value == value)

  def withValue(value: String): JweAlg =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"JweAlg enum case not found: $value"
      )
    )

  given decoderJweAlg: Decoder[JweAlg] = Decoder.decodeString.map(withValue)
  given encoderJweAlg: Encoder[JweAlg] =
    Encoder.encodeString.contramap[JweAlg](_.value)
