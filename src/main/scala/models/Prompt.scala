...
object Prompt {
  ...
  def withValueOpt(value: String): Option[Prompt] =
    Prompt.values.find(_.value == value)

  def withValue(value: String): Prompt =
    withValueOpt(value).getOrElse(
      throw java.lang.IllegalArgumentException(
        s"Prompt enum case not found: $value"
      )
    )

  given decoderPrompt: Decoder[Prompt] = Decoder.decodeString.map(withValue)
  given encoderPrompt: Encoder[Prompt] =
    Encoder.encodeString.contramap[Prompt](_.value)
